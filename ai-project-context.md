...
- 预算明确改口采用本次金额；相互冲突的上限或未解析的预算范围先澄清，不当作无预算推荐。
- 画像提取单独关闭 thinking、默认最多 2048 输出 Token；2 个并发工作位、无等待队列，繁忙或超时降级，不延长商品节点 500ms 等待。
- Router DNS 失败仅在 HTTP 发送边界证明未发送时标为 FAILED；读超时等仍为 UNCERTAIN，禁止自动重放。
