...
| 🔗 **P3 跨模块推荐服务** | 新增 `smart-assistant-recommend`(端口 8089)，Feign 调用 Product(图谱)+Order(购买历史)。三策略：图谱推荐→协同过滤→热门兜底。推荐引擎延迟 **0.120ms** |
...
| **Recommend** | **8089** | **🎯 P3 跨模块推荐服务**。三策略推荐引擎：图谱推荐(ProductGraphService)→协同过滤(OrderGraphService 用户购买历史)→热门兜底。Feign 调用 Product+Order REST API，**含 FallbackFactory 熔断降级**(ProductFeignClientFallbackFactory/OrderFeignClientFallbackFactory 返回空列表避免上游异常) |
| **Tool-Registry** | — | **🎯 MCP 兼容工具注册中心 + 发现聚合网关**。中心目录管理所有 `@Tool` 与 MCP-backed 工具（SQL 等）的注册/状态/标签/风险/能力元数据；以 MCP Server（WebMvc/SSE）暴露 `tools/list` 全量发现 + `search_tools` 能力过滤。作为 MCP **client** 拉取后端 MCP Server 工具列表，同步进中心目录（`McpToolSourceIngestor`，含 `inputSchema`/`endpoint`/命名 `namespace.toolName` 防冲突）。自身**不实现**传递式 `tools/call`（执行由 Agent 侧 `McpBackendToolExecutor` 经 `ToolGateway` 治理链转发） |
