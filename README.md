...
| 模块 | 测试数 | 覆盖内容 |
|------|--------|---------|
| common | 57 | SQL 安全校验器、中文分词器（IKAnalyzer/HanLP）、同义词、词性标注、意图识别、缓存、EntityProfileService、**BgeReranker(降级策略)**、**SECURITY_SCRIPT_TIMEOUT/SECURITY_SCRIPT_RESOURCE_LIMIT**、**AgentMemoryService 集成测试(6用例: 多轮对话/老化警告/截断/隔离/文件格式)** |
