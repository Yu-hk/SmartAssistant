...
> **Pipeline RAG 检索管线**、**双层拦截器架构**（ServiceInterceptor + Spring AI Advisor）、**Skill 技能包系统**、
> **SSRF 防护**、**SSE 事件框架**、**多数据库方言**、**管理端 API**、**53+ 单元测试**。
...
| 📋 **SSE 事件框架** | `SseEvent` + `SseEventBus` 替代内联 SSE 构建，支持 Redis 缓存/断线续传/代理转发。StreamChatController 660→200 行 |
| 🗄️ **多数据库方言** | `DatabaseDialect` 接口 + PostgresDialect/MySQLDialect，12 个方言方法自动适配 PG/MySQL 语法差异 |
| 🖥️ **管理端 API** | `AdminController` 补齐 9 个管理端点（stats/sessions/faq/check-login/env-config），Express 和 Java 双实现 |
| 🧪 **53+ 单元测试** | 覆盖 Pipeline/Advisor/Skill/SSRF/SSE/PromptBuilder 等全部新增模块 |
