...
| 📊 **P0 检索链路追溯 (RetrievalTrace)** | 完整记录 query → variants → 各路召回(向量/BM25/Rerank每步docId+title+score) → RRF融合 → 最终入Prompt上下文。Redis TTL 1h，支持 `findByRequestId()` 线上查根因。`RetrievalFailureAnalyzer` 自动分析失败原因 |
| 🔀 **P1 Multi-Query + Step-back + HyDE 查询扩展** | `MultiQueryService` 三种扩展策略：① Multi-Query(3-5角度变体) ② Step-back(先问背景再答细节) ③ HyDE(先生成假设答案再检索)。`ExpansionStrategy` 枚举统一入口，RRF 融合多路结果 |
| 🧩 **P1 Parent-Child 双粒度分块** | 子块 256 tokens(向量检索用)+ 父块 1024 tokens(LLM阅读用)。子块通过 `parentDocId` 关联父块，检索命中子块→定位父块送 LLM。`KnowledgeIngestionService` 可选启用 |
