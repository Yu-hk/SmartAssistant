...
| 📊 **全栈可观测** | Micrometer + Prometheus + Grafana 指标，Jaeger 链路追踪，Loki 日志聚合，10 个自定义仪表盘（含 Token 累计 Stat + 业务仪表盘 Token 面板）；Order/Product/General 各服务独立暴露 `a2a_llm_token_input_total` / `a2a_llm_token_output_total` 等 8 类 Agent 指标 |
...
| **General** | 8087 | 闲聊问答，新闻热点，单位转换(温度/长度/重量/货币)，**沙箱保护的多步脚本执行**(ScriptSandbox 三层防御: 关键字黑名单/资源限制/超时熔断)，支持风格切换 |
