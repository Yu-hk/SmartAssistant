...
| 📊 **P0 Agent 评测框架** | `AgentEvaluationResult`(Builder模式,5维度:意图匹配/工具准确率/回复质量/效率/综合) + `EvaluationReportService`(黄金测试集管理+批量评测+历史追踪)。配套 JSON 黄金测试集(4 RAG + 6 Agent 用例) |
...
| 🚪 **评测闭环门禁 (CI)** | `EvalGateConfig`+`EvalBaseline`+`EvalGate`+`EvalReportExporter` 四件套 + `GoldenSuiteEvalGate` 编排器，双保险机制：**绝对阈值**(防断崖) + **基线回归**(±0.05 防渐变)。`.github/workflows/eval-gate.yml` 在 push/PR 到 main 时自动 compile-all + 跑黄金集评测，首次自举生成 `eval-baseline.json`，门禁不通过则 CI 失败阻断合入 |
