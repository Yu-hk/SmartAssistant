# SmartAssistant — 多智能体对话系统

[![Java](https://img.shields.io/badge/Java-21%2B-blue)](https://adoptium.net/)
[![Spring Boot](https://img.shields.io/badge/Spring%20Boot-3.4.8-brightgreen)](https://spring.io/projects/spring-boot)
[![Spring AI](https://img.shields.io/badge/Spring%20AI-2.0.0-brightgreen)](https://docs.spring.io/spring-ai/reference/)
[![React](https://img.shields.io/badge/React-18-61DAFB)](https://react.dev/)
[![License](https://img.shields.io/badge/License-MIT-yellow)](LICENSE)
[![Production Readiness](https://img.shields.io/badge/production_readiness-audited-blue)](docs/production-readiness-checklist.md)

> 基于 Spring AI 2.0.0 + 多智能体协作平台，集成 **DeepSeek V4-Flash 云端推理**（通用对话/订单/商品 Agent）+ **本地 Ollama 推理引擎**（deepseek-r1:7b 主模型/Router 路由）+ **轻量 Ollama 通道**（qwen2.5:3b 辅助任务）。
> 支持多 Agent 协同、三层路由兜底、订单查询、商品咨询、语义缓存、全链路监控、
> **Pipeline RAG 检索管线**（7 Handler + 自适应权重 + 去重 + 重排序）、
> **双层拦截器架构**（ServiceInterceptor + Spring AI Advisor）、**Skill 技能包系统**、
> **SSRF 防护**、**SSE 事件框架**（心跳/超时/限流/续传）、**多数据库方言**、
> **管理端 API**、**分级人工确认**、**Agent 幂等机制**、**冷却窗口摘除**、**多级 Token 预算**、
> **查询复杂度分类器**、**Gateway 审计**、**300+ 单元测试**、
> **RAG 评测框架**（Recall@K/MRR/nDCG/hallucination 检测）、**Agent 评测框架**（5 维度评分）、
> **LangGraph 对标 Graph 引擎**（Checkpoint 持久化/条件边/Graph 循环/HITL/类型化 State/Handoff Checkpoint）。

---

## 目录

- [快速导航](#快速导航)
- [项目简介](#项目简介)
- [核心亮点](#核心亮点)
- [系统架构](#系统架构)
- [技术栈](#技术栈)
- [服务说明](#服务说明)
- [环境要求](#环境要求)
- [快速开始](#快速开始)
  - [1. 启动基础设施](#1-启动基础设施)
  - [2. 配置环境变量](#2-配置环境变量)
  - [3. 构建项目](#3-构建项目)
  - [4. 启动服务](#4-启动服务)
- [详细部署流程](#详细部署流程)
- [RAG 召回管道](#rag-召回管道)
- [Router 反思器](#router-反思器)
- [生产级 Agent 架构实战落地（G1/G2）](#生产级-agent-架构实战落地g1g2借鉴陈嗣俊2026-06-04)
- [G3 Tier 多模型路由（统一模型接入层）](#g3-tier-多模型路由统一模型接入层)
- [P1/P2 Loop Engineering 确定性约束](#p1p2-loop-engineering-确定性约束对标文章⑥)
- [G4 观测看板固化](#g4-观测看板固化)
- [P0 熔断半开 + 多级优先级队列](#p0-熔断半开--多级优先级队列)
- [P1 小模型前后置 + 灰度发布](#p1-小模型前后置--灰度发布)
- [P2 执行 DAG 可视化 + 分组完成率看板](#p2-执行-dag-可视化--分组完成率看板)
- [RAG 索引版本化 + 对账服务](#rag-索引版本化--对账服务)
- [@Tool 统一日志切面](#tool-统一日志切面)
- [语义缓存](#语义缓存)
- [请求排队](#请求排队)
- [配置说明](#配置说明)
- [监控体系](#监控体系)
- [API 文档](#api-文档)
- [常见问题](#常见问题)
- [开发指南](#开发指南)

---

## 快速导航

> 本文档较长，含大量阶段性改造记录。**新读者只需阅读本节 + [系统架构](#系统架构) + [快速开始](#快速开始)**；各功能模块实现细节见下方对应章节；历史迭代日志见 [docs/changelog.md](docs/changelog.md)。

### 5 分钟本地启动

```bash
# 1. 启动基础设施（Redis / Nacos / PostgreSQL / Zipkin）
docker compose -f docker-compose.dev.yml up -d redis nacos postgres zipkin

# 2. 构建（使用兼容性脚本，规避 Windows 编码问题）
bash mvn21.sh clean package -DskipTests

# 3. 启动服务（推荐在 IDE 逐个启动；端口见下表）
#    最小可用组合：gateway(8081) + consumer(8082) + router(8083) + user(8086)
```

完整步骤见 [快速开始](#快速开始) 与 [详细部署流程](#详细部署流程)。

### 端口分配表

| 服务 | 端口 | 说明 |
|------|------|------|
| Gateway | 8081 | API 统一入口（JWT / 限流 / 负载均衡） |
| Consumer | 8082 | SSE 流式对话核心入口 |
| Router | 8083 | 多 Agent 路由 / 意图分析 |
| Product | 8084 | 商品 Agent |
| Order | 8085 | 订单 Agent（Text-to-SQL） |
| User | 8086 | 用户服务（注册 / JWT 签发） |
| General | 8087 | 通用助手 Agent |
| Recommend | 8088 | 跨模块推荐服务 |
| **Tool-Registry** | **8090** | MCP 兼容工具注册中心（P0 由 8088 迁至 8090，避开与 recommend 冲突） |
| Embedding Service | 8091 | BGE ONNX 向量化服务（1024 维） |
| Redis | 6379 | 缓存 / 语义缓存 / SSE 断线续传缓冲 |
| Nacos | 8848 / 9848 | 服务发现（HTTP / gRPC） |
| PostgreSQL | 5433 → 5432 | 业务库（pgvector 扩展） |
| Zipkin | 9411 | 链路追踪 |
| Prometheus | 9090 | 指标采集 |
| Grafana | 3000 | 观测看板 |

> 各服务的完整职责说明见 [服务说明](#服务说明)。

---

## 项目简介

SmartAssistant 是一个多智能体对话系统，基于 **Spring AI** 框架和 **自研 ReAct 循环**实现。系统通过 **智能路由器** 将用户请求分发到不同的专业 Agent，支持：

- **Order Agent**：订单查询、退款处理、物流跟踪、优惠券查询、知识库检索
- **Product Agent**：商品查询、库存查询、价格查询、知识库检索
- **General Agent**：闲聊陪伴、问答、新闻热点、天气查询、单位转换、汇率转换
- **Embedding Service**：本地 BGE ONNX 嵌入服务（1024维），供语义缓存、知识库检索和经验匹配
- **Router 决策链路**：经验匹配(TOOL/COMMON) → 语义缓存(T1/T2/T3) → 任务分析(意图/实体/约束/风险/工具评分) → 规则层评测后处理(实体归一/词槽/澄清/置信度) → 意图查询改写 → DAG 多Agent协作 → inlineFallback(三级兜底)
- **Consumer 聚合**：统一对话入口，上下文管理、用户画像、会话记忆沉淀

采用本地推理架构（Ollama + ONNX Runtime），零外部 API 依赖。通过 **Prometheus + Grafana + Jaeger + Loki** 实现全链路可观测性。

---

## 核心亮点

| 特性 | 说明 |
|------|------|
| 🧠 **多 Agent 协作** | 9 个微服务通过 Nacos 自动发现与注册。Router 支持任务分解 → 并行调用 → 结果合并，跨域问题自动分配多个 Agent |
| 🧠 **三层语义缓存 + BGE ONNX + 智能跳过** | 精确匹配 → 关键词哈希 → BGE 向量匹配。短时效 Agent(TTL<1h)回复自动跳过缓存，管理员工具操作通过 Redis 标记跳过，均保留路由决策加速 |
| 🔍 **12 维 Agent 可标注评测** | LLM 输出后自动执行 6 层规则后处理：实体归一化(日期/金额/地点/纠错) → 词槽状态机(6意图词槽表+缺失/冲突检测) → 澄清判断(追问生成) → 输入鲁棒性(错别字/别名纠错)。叠加 LLM 层 6 维（意图识别、多意图拆分、隐含意图、拒识、实体识别、工具评分），完整覆盖文章推荐的 12 个评测维度 |
| 🎯 **意图置信度 + 阈值路由** | LLM 输出意图置信度(0.0~1.0)，低于 0.6 阈值时自动触发澄清流程而非盲目执行，避免误操作 |
| 🔄 **意图引导的查询改写** | 任务分析后根据意图类型选择改写策略：多意图→查询分解、模糊→查询扩展(补充实体)、精确→保留、对话→指代消解 |
| 🗂️ **多样性 RAG + 生产级增强** | PDF/Word/HTML 文档解析路由 + 语义/递归分块(Chunking) + 5路召回+RRF融合 + BM25混合评分 + 分级检索策略(LIGHT/STANDARD/DEEP) + 版本控制(自动淘汰旧版) + ACL 租户隔离 + INSUFFICIENT_EVIDENCE 规范拒答 |
| 🧩 **P0 LLM 调用网关** | `AgentLLMGateway` 统一封装 ChatClient 调用，内置超时(虚拟线程Future.get)、指数退避重试、熔断保护(5次失败→30s恢复)、Token 计量。与模型 SDK 解耦(LLMExecutor函数式接口) |
| 🔑 **P0 统一工具治理** | `ToolRegistry` + `ToolGateway`：工具注册→鉴权(Scope)→熔断(3次失败)→限流(令牌桶)→幂等(IdempotencyKey)→审计日志全链路。38 个 @Tool 全部注册，9 个高风险工具需审批 |
| 🧠 **P0 工具能力分类 + MCP 发现（T1→T2e）** | 新增 `functionalCapabilities` 字段分离**风险/访问维度**(已有)与**功能性维度**(业务语义，如 `query-order` / `refund` / `sql-query`)，32 枚受控令牌。`smart-assistant-tool-registry` 独立服务作为 **MCP 兼容聚合网关**（WebMvc/SSE），暴露 `tools/list` + `search_tools`, 接入后端 MCP Server（SQL 等）。Agent 侧 `McpBackendToolExecutor` 转发 `tools/call` 100% 经 `ToolGateway` 治理链（status 拦截 / 审批 / 限流 / 超时 / 审计，**P0 接线不破**）。Agent 可在运行期通过 `discover_tools` CORE 元工具按能力名自主发现并加载预载集之外的工具（护栏杆：去重/每轮1次/每会话10次/最多15动态工具/Registry挂时优雅降级）|
| 🔀 **P0 多 Agent 通信可靠性三层防御** | ① RestTemplate 超时统一化(Consumer→Router / Router→Agent 均配置 3s connect + 5s read timeout，消除"可靠性与延迟死锁") ② Router→Agent Resilience4j 熔断器(50%失败率→30s半开→fallback降级) ③ DAG 节点级熔断(连续失败≥2次跳过该Agent后续节点) |
| 🔍 **P1 Agent 执行状态机** | 10 状态(→WAITING_TOOL/WAITING_APPROVAL/FAILED/DEAD_LETTER) + 11 事件类型。`AgentEventBus` Redis List 持久化执行轨迹，`GET /events/{requestId}` 端点可查询每次路由的完整决策链 |
| 📋 **P1 子任务错误分类（ErrorType）** | SubTaskResult 增加 NONE/RETRYABLE_FAILED/FATAL_FAILED/NEED_REPLAN 四状态错误分类。GraphExecutionService 根据 ErrorType 差异化处理：RETRYABLE → 指数退避重试(1s→2s→4s, 最多3次), FATAL → 记录跳过, NEED_REPLAN → 触发重规划生成新子任务图 |
| 📉 **P1 异常率阈值降级（DegradationService）** | 基于 Redis Sorted Set 的 60s 滑动窗口错误率计数器，错误率>20%跳过复杂DAG编排降级为单Agent调用，>40%回退内联兜底。最小10次调用后触发(防冷启动误判)。配合 EventBus 幂等去重(SETNX 防重入队死循环)，切断"异常处理制造异常"的负反馈循环 |
| ✅ **P1 验收标准（successCriteria）** | SubTask 新增 successCriteria 字段，TaskPlanner prompt 升级为5字段格式(id|desc|agent|deps|验收标准)。ReflectionService 新增 checkCriteria()，LLM 判断输出是否满足验收标准，不满足时标记 NEED_REPLAN |
| 🔄 **P1 重规划机制** | GraphExecutionService 每轮执行完毕后检查 NEED_REPLAN 节点，调用 TaskPlannerService.replan() 生成新子任务图，通过 IntentGraph.addNodes() 动态追加到执行队列 |
| ⏱️ **P1 用户画像时间衰减** | UserProfile 新增指数衰减模型 weight×e^(-λ·days)，30天后旧偏好降至 74%。支持自定义衰减率参数，clamp≥0.1 防止零权重 |
| 🔗 **P2 BGE 语义合并（Semantic Merge）** | ExperienceService 在 Jaccard 关键词去重（0.35）基础上增加 BGE 向量语义合并（cosine≥0.85）。Milvus/pgvector/全量遍历三路降级，自动合并语义等价但关键词不同的经验条目 |
| 📜 **P2 摘要回指（Back-Reference）** | RouterRagService 从对话历史提取 ORDER_ID/PRODUCT/AMOUNT/DATE/ACTION/DECISION 六类实体，生成结构化回指文本 `【历史引用】` 注入增强问题。配置项 `backref-enabled=true` |
| 📊 **P2 监控体系增强** | ToolLogAspect 和 NewMetricsCollector 新增 3 类指标：错误分类计数器(retryable/fatal)、延迟百分位 Timer(p50/p95/p99)、按工具+错误类型的 `a2a_tool_error_type` Counter |
| 🔍 **P0 RAG 变更检测闭环** | KnowledgeIngestionService 集成 ContentHashCache + HashUtil，摄入前按 baseDocId 计算聚合 SHA-256 哈希，通过 `normalize_text()` 过滤时间戳/页脚假变更，未变更文档跳过解析/分块/嵌入/入库全链路 |
| ⚡ **P2 关键词快车道** | 高频明确意图(退款/查订单/问候等)跳过 LLM 分诊直接路由。**7.3μs/次**，吞吐量 132,287 次/秒，加速比 ~68,000x |
| 🗣️ **P2 对话阶段感知** | `RouterStageAwareService` 四阶段(GREETING/COLLECTING/PROCESSING/CONFIRMING)聚焦 Prompt 注入，集成到 TaskAnalysisService。降低 Token 消耗，减少跨阶段上下文干扰 |
| 🔗 **P3 商品知识图谱** | `ProductGraphService` 15 节点/36 边的内存商品关系图(同类/配件/替代/升级/互补)。通过 Pipeline GraphSearchHandler 集成。查询延迟 **1.947μs** |
| 🔀 **P1 ProductRagService Multi-Query 扩展** | 5 路召回前新增 Step 0 Multi-Query 查询扩展（需配置 `product.rag.multi-query.enabled=true`），生成 3 个角度变体，每变体走 5 路召回后统一 RRF 融合，提升召回覆盖率 |
| 🔄 **P0 RAG Pipeline 管线化** | 借鉴 Snail AI 的 Pipeline 设计，7 个独立 Handler 按 Order 编排：MultiQuery(0)→QueryRewrite(2)→ExactMatch(10)→KeywordSearch(20)→BM25(30)→KnowledgeSearch(40)→GraphSearch(50)→RrfFusion(100)→Dedup(105)→Rerank(110)。可插拔、可配置、可测试 |
| 🧠 **P0 查询重写(QueryRewrite)** | `QueryRewriteHandler` 利用 deepSeekChatModel 将口语化查询改写为检索友好形式（扩展缩写/补充同义词/书面化），Order=2 在 Multi-Query 之后执行 |
| 🎯 **P1 嵌入式重排序(Rerank)** | `RerankHandler`(Order=110) + `EmbeddingScorer` 通过嵌入服务余弦相似度对 RRF 融合结果二次精排，BGE 标题+全文加权评分 |
| 🧹 **P1 检索去重(Dedup)** | `DedupHandler`(Order=105) 双模式：EXACT(SHA-256 精确匹配) + AGGRESSIVE(Jaccard 字符 3-gram 重叠度>85%模糊去重) |
| 📦 **P2 Skill 技能包系统** | 参考 Snail AI Skill 设计。`SkillPackage` + `SkillPackageManager` + YAML 配置加载。技能包绑定 Agent 后自动注入 system prompt。Product 模块 3 个示例技能包(比价/库存预警/知识问答) |
| 🔄 **双层拦截器架构** | Layer 1: `ServiceInterceptor` SPI(Order 控制，PerformanceMonitor + Logging) 替代 4 份重复切面；Layer 2: Spring AI `Advisor` 链(PromptAudit 100 + TokenUsage 350 + ThinkingCollector 400)，包裹 ChatClient 调用全流程 |
| 🛡️ **SSRF 防护** | `UrlSecurityValidator` 校验内网IP/DNS/localhost/域名后缀，白名单/黑名单模式 |
| 📋 **SSE 事件框架** | `SseEvent` + `SseEventBus` 替代内联 SSE 构建，支持心跳ping(15s)/闲置超时(60s)/单用户限流(3并发)/Redis 缓存/断线续传。StreamChatController 660→200 行 |
| 🗄️ **多数据库方言** | `DatabaseDialect` 接口 + PostgresDialect/MySQLDialect，12 个方言方法自动适配 PG/MySQL 语法差异 |
| 🖥️ **管理端 API** | `AdminController` 补齐 9 个管理端点（stats/sessions/faq/check-login/env-config/permission-response），Express 和 Java 双实现 |
| 🔐 **分级人工确认** | `PermissionBridgeService` Redis 桥接 Agent 端审批与前端 InlinePermissionCard 弹窗，120s 超时自动取消 |
| 🔒 **Agent 幂等机制** | `IdempotencyService` Redis 分布式锁 + 任务状态机（PENDING→RUNNING→SUCCESS/FAILED）+ 全局 TaskID |
| 📊 **RAG 评估指标** | `MetricsCollectorHandler` (Order=1000) 采集检索耗时/召回数/质量分，Micrometer → Prometheus |
| 🧪 **53+ 单元测试** | 覆盖 Pipeline/Advisor/Skill/SSRF/SSE/PromptBuilder/去重/重排序 等全部新增模块 |
| 🧊 **冷却窗口自动摘除** | `AgentCooldownService` Redis 计数失败次数，连续 3 次达阈值自动进入冷却窗口（默认 5 分钟），冷却期间不参与路由 |
| 💰 **多级 Token 预算** | `TokenBudgetService` L1 全局/L2 App/L3 用户三级每日预算控制，超限软降级不硬拒 |
| 📊 **成本看板** | `GET /api/admin/costs` 近 7 天按 Agent 汇总 + 每日趋势 + 概览 |
| 🧠 **查询复杂度分类器** | `QueryComplexityClassifier` 纯规则实现 <1ms，3 级分类 SIMPLE/MEDIUM/COMPLEX，推荐对应模型层级 |
| 📋 **Gateway 审计过滤器** | `RequestAuditFilter` 统一注入 X-Request-Id、记录方法/路径/耗时/状态码、慢请求 >5s 告警 |
| 📊 **P0 RAG 评测框架** | `RetrievalMetrics`(Recall@K/Precision@K/MRR/nDCG) + `HallucinationDetector`(4层规则:数字断言/否定反转/日期/实体) + `RAGEvaluator`(编排器) + `RAGEvaluationResult`(综合评分+批量报告)。单次调用整合检索指标+Faithfulness+幻觉检测 |
| 📊 **P0 Agent 评测框架** | `AgentEvaluationResult`(Builder模式,5维度:意图匹配/工具准确率/回复质量/效率/综合) + `EvaluationReportService`(黄金测试集管理+批量评测+历史追踪)。配套 JSON 黄金测试集(4 RAG + 6 Agent 用例)。**四缺口已补齐**：Trial/pass^k(`TrialRunner`+`PassKCalculator`)、Grader 瀑布流(`grader/` 规则→LLM-Judge+3大偏差防护)、人工路由(`HumanReviewRouter` 2%抽检+边界升级)、根因分析(`RootCauseAnalyzer` 5步聚类定责)，由 `EvalPipeline` 串成链路 |
| 🔒 **P0 Graph Checkpoint 持久化** | `GraphCheckpointService` 将 completedMap/breakerFailureCounts/执行轮次持久化到 Redis(TTL 30min)。`executeWithResume()` 自动恢复 Checkpoint 继续执行。执行完成后自动清理 |
| 🔀 **P0 条件边(Conditional Edge)** | `ConditionType` 枚举(RESULT_CONTAINS/NOT_CONTAINS/SUCCESS/FAILED)。`ConditionalDep` 声明式条件依赖。运行时 `getExecutableNodes(completedNodeIds, nodeResults)` 根据源节点结果评估条件。对标 LangGraph `add_conditional_edges` |
| 🔄 **P1 Graph 级别循环执行** | `rerouteTarget` 重路由字段，条件满足时跳转到前驱节点形成循环。`maxGraphIterations` 预算(默认3轮)+ `nodeExecutionCounts` 每节点最多3次。死锁检测兼容重路由。对标 LangGraph Q7 循环执行 |
| ⏸️ **P1 HITL 集成(Graph 引擎)** | `IntentNode.humanApprovalRequired` 标记。Graph 执行到 HITL 节点时自动保存 Checkpoint 并暂停。`AgentExecutionState.WAITING_APPROVAL` 状态已预留。对标 LangGraph interrupt/resume |
| 📦 **P2 GraphState 类型化 State** | 三种 Reducer 模式：`OVERWRITE`(覆盖)/`APPEND`(追加)/`MERGE_MAP`(聚合)。消息流 `addMessage()/getMessages()` 追加模式。节点执行记录追踪 `recordExecution()/getRecordsForNode()`。对标 LangGraph `MessagesState` + Reducer 语义 |
| 💾 **P2 Handoff Checkpoint** | `executeWithResume()` 每轮 Handoff 后自动保存 Checkpoint(Key 前缀 `handoff:`)。执行完成后自动清理。解决 Handoff 链服务重启状态丢失 |
| 🔧 **P1 工具返回值规范化** | 修复4处 tools 返回值不一致：`GeneralTools.convertTemperature()` default 分支、`ImageTools` API Key 缺失、`GeneralMemoryTool` null 校验+tray-catch。全部改为 `ToolResult.error()` 结构化格式 |
| 🔄 **P0 SSE 断线续传** | Consumer `forwardSSE()` 逐行解析 SSE 事件 → 注入 `id: seqNo` → Redis Sorted Set 缓冲(`ZADD` score=seqNo)。EventSource 自动重连 + `Last-Event-ID` 协议。续传查询 O(log n) (`ZRANGEBYSCORE`) |
| 🛑 **P1 请求取消（立即释放 GPU）** | 前端 `handleStop()` 关闭 EventSource 连接，后端检测到写入异常后立即释放 LLM 槽位(slots.release)并关闭 Agent HTTP 连接。冗余保障 `POST /chat/cancel`。提前中断节省 **10~20s/次 GPU 推理** |
| 📤 **P1 异步 Agent 事件总线** | `AgentEventBus` 基于 Redis List 的异步通信(RPUSH+BLPOP)，Router 发布事件到 `agent:events:{agentName}`，Agent 虚拟线程异步消费。失败自动重新入队(含 **SETNX 幂等去重**，同一事件仅重入一次，切断重试死循环)。通过 `router.graph.async-handoff-enabled` 启用 |
| 🚦 **P2 三层请求排队流控** | L1: 全局 Semaphore 并发控制(默认5)。L2: 会话级并发(每sessionId最多1请求)。L3: PriorityBlockingQueue 优先级调度(`queue.priority-enabled`)。配置 `queue.session-max-concurrency` |
| 🔗 **P3 跨模块推荐服务** | 新增 `smart-assistant-recommend`(端口 8088)，Feign 调用 Product(图谱)+Order(购买历史)。三策略：图谱推荐→协同过滤→热门兜底。推荐引擎延迟 **0.120ms** |
| 🧩 **L3 三路并行意图融合** | 规则(KeywordFastRoute, 7μs) ∥ 小模型(BGE余弦分类器, 10个意图类, 10ms) ∥ LLM(TaskAnalysis, 200ms) 三路并行融合。融合策略：规则≥0.9直接输出→小模型≥0.7优先→LLM兜底。预期减少 **~90% LLM 调用** |
| 🎯 **Intent Prior Bias 先验偏置** | ORDER/PRODUCT/COMPLEX 意图增加 0.15~0.05 先验偏置，抑制 GENERAL 兜底意图的排序偏置。偏置值小仅边界 case 翻转，不压倒强信号 |
| 🔄 **L5 意图漂移检测** | 多轮对话中检测用户意图漂移：BGE 余弦相似度对比当前轮与上轮用户问题。双阈值：<0.80 漂移(触发新意图识别)、<0.55 强漂移(标记上下文需重置)。集成到 Router Step 3.5 |
| 🔀 **Step 0 多意图直接并行调度** | 经验匹配检测到 multi-intent 时(secondaryIntents非空)，不再走 LLM 管道，直接虚拟线程并行调度多个 Agent 并合并结果。延迟从 `经验匹配+LLM×2+Agent串行` 降为 `经验匹配+max(Agent₁,Agent₂)` |
| ⚙️ **P4 Hot Agent + 调度系统** | 基于 Redis List 的异步任务队列(LPUSH/BRPOP)，虚拟线程 Worker 池，Hot Agent 预热(Order/Product/General/Recommend 常驻内存+并发限流)。`scheduler.enabled=true` 开启，默认关闭 |
| 🛡️ **AST 级 SQL 防护** | 基于 jsqlparser 的表名白名单校验，精确到 SQL AST 节点，杜绝注入 |
| 🔍 **@Tool 统一日志切面** | AOP 拦截全部 @Tool 方法，自动记录 requestId + 输入参数 + 输出结果 + 执行耗时；requestId 通过 MDC 全链路透传（Consumer → Router → Agent → @Tool） |
| 🔄 **二阶段质量评估** | 反射器(ReflectionService)纯规则五维评分 + LLM-as-Judge(QualityEvaluationService)四维语义评估；反射器通过后仅在边界区间触发 LLM 质检，平衡质量与开销。质量阈值从 0.05 提升至 0.30，拒答统一为 INSUFFICIENT_EVIDENCE 结构化格式 |
| 📋 **标准错误码 + 表驱动恢复** | `AgentErrorCode` 统一枚举 32 个错误码(6 分类)，`ErrorRecoveryService` 三态表驱动恢复路由(RETRY/RETRY_BACKOFF/CLARIFY_USER/FALLBACK_AGENT/TERMINATE)；集成到 SmartReActAgent 6 个错误点 + 全工具 45 处调用 |
| 🔄 **Agent 级工具自动重试** | `executeToolCallWithRetry()` 统一并行/串行工具执行路径，解析 error_code JSON 自动重试，退避策略由 ErrorRecoveryService 表驱动决策 |
| 📊 **全栈可观测** | Micrometer + Prometheus + Grafana 指标，Jaeger 链路追踪（含嵌套 Span：`agent-llm-call` + `agent-tool-execute`），Loki 日志聚合，10 个自定义仪表盘（含 Token 累计 Stat + 业务仪表盘 Token 面板）；Order/Product/General 各服务独立暴露 `a2a_llm_token_input_total` / `a2a_llm_token_output_total` 等 8 类 Agent 指标。新增 **错误分类监控**（retryable/fatal 分位数 + 延迟 p50/p95/p99） |
| ⏳ **请求排队 + SSE 流式** | Semaphore 限流 LLM 并发(默认5)，排队时 SSE 实时推送位置，支持 thinking/tool_call/response 事件 |
| 🐳 **容器化部署** | Dockerfile + docker-compose.deploy.yml，7 个服务一键构建部署 |
| 🔧 **自研 ReAct 循环** | SmartReActAgent 实现全可控 ReAct 循环：迭代限制(**G2 入口分级**，默认10轮)、超时保护(60s)、Token 预算追踪(80%)、上下文压缩(9段式摘要+增量更新+MicroCompact)、**G1 工具同参数去重**(FNV-1a 64-bit 指纹防接口500后风暴)、并行工具执行(最大4并发) |
| 🎯 **G2 入口分级（ReActProfile）** | 不可变 `ReActProfile` 配置模型 + `ReActProfileRegistry` + `@ConfigurationProperties(prefix="smartassistant.react")`。按入口 `order/general/product/mcp` 声明式差异化 maxIterations/timeoutMs/tokenBudgetRatio/contextWindow/toolTimeoutMs/maxConcurrency；未配置严格回退 DEFAULT(10/60s)，向后兼容零行为变化 |
| 🔁 **G1 工具同参数去重** | `SmartReActAgent` 连续无增量检测哈希键由工具名扩展为 `name(argHash64(args))`，FNV-1a 64-bit 参数指纹；同工具同参数连续重复触发停止，防止接口 500 后工具调用风暴；参数不同(即使同工具名)视为有效进展 |
| 🏗️ **G3 Tier 多模型路由（统一模型接入层）** | `TieredModelRouter` 复用 `QueryComplexityClassifier` 按复杂度动态选档(SIMPLE→LIGHT/MEDIUM→STANDARD/COMPLEX→HEAVY) + 意图覆盖强制升档 + 失败沿降级链平滑降级(HEAVY→STANDARD→LIGHT,LIGHT永为兜底) + Micrometer 可观测(档位分布/降级次数/时延)。集成到 `RouterService.inlineFallback()` 替代固定 `lightChatClient`。修复 `ModelRouterService` 死代码(依赖不存在的bean→委托common TieredModelRouter)。[详见↓](#g3-tier-多模型路由统一模型接入层) |
| 🛡️ **P1 Pre-AL Gate 执行契约** | 每轮迭代前注入结构化执行契约到 SystemMessage：当前轮次(x/maxIterations)、当前阶段、未通过验收检查清单、4条硬规则(不得跳步/不得空口宣称完成/不得跳过检查/阻塞时明确请求暂停)。纯代码生成完全确定。[详见↓](#p1p2-loop-engineering-确定性约束对标文章⑥) |
| 🔀 **目标连续性裁决** | `GoalContinuityArbiter` 两级裁决：确定性规则(继续/确认→同任务，算了/重来→新任务)→词汇重叠度(Jaccard 0.24延续/0.08新任务)→LLM灰色区(置信度<0.72→FUZZY交用户)。中文逐字+英文小写词双通道词汇化 |
| 📋 **反馈学习日志** | `FeedbackLog` per-round 结构化反馈：mistake_pattern/avoid_next_time/suggested_strategy。最多保留20条。SmartReActAgent 在超时/迭代上限/正常完成时自动记录 |
| 📊 **G4 观测看板固化** | `monitoring/provisioning/dashboards/a2a-daily-ops.json`：6 面板覆盖请求量&成功率/延迟(p50/p95/p99)/Token消耗/Agent健康/路由质量/错误分类。导入 Grafana 即用 |
| 📐 **G5 KB 切块尺寸合规** | `ChunkStrategy.defaultMaxTokens()` 512→1024，`defaultOverlap()` 50→128，对齐文章 800-1200 中文汉字/chunk 推荐值 |
| 🔍 **P1 LoopGuardService 确定性快速判断** | Agent 输出后立即纯代码检测三类状态：阻塞标记(blocked/无法继续/需要授权)、用户决策请求(请选择/是否继续)、基础设施错误(timeout/500/rate limit)。11组Pattern覆盖80%异常场景，命中即暂停无需LLM评估 |
| 🚪 **P1 Phase Gate 验收门禁** | 4种代码级验收(file_exists/file_glob_count/script_check/user_confirmation)，全部纯代码执行。代码可否决 LLM 的 done 判定——LLM 说完成了但文件没生成=未通过 |
| 🔄 **P2 决策状态机 10 条优先级链路** | 每轮迭代末调用 `resolveNextAction()` 10路优先级链：FINALIZE→ADVANCE_PHASE→PAUSE_BLOCKED→PAUSE_INFRA→AWAIT_CONFIRMATION→PAUSE→PARSE_FAILURE→ITERATION_BUDGET→STRATEGY_SWITCH→CONTINUE。日志记录决策路径 |
| ⛔ **P1 连续解析失败保护** | 连续3次LLM返回空结果→暂停返回错误，避免用坏掉的评估器烧光预算 |
| 📉 **P1 无进展计数器** | 连续3轮无工具调用且回复<50字符→提前终止。STRATEGY_SWITCH阈值=2轮（低于终止阈值1步，提前触发策略切换日志） |
| 🔄 **P1 摘要优先模式** | `ResultMerger` 上下文构建用 summary(200字符)替代全量 result，完整结果存入 Redis。节省 Token 开销 |
| 🔄 **Graph 节点级 SSE 事件** | 5 类 SSE 事件(node_started/completed/failed/skipped/replan)写入 `routing:sse:events`，前端实时看到每步进度 |
| 🔄 **P0 熔断半开恢复** | `DegradationService` 三态(关闭/半开/打开)。错误率降至阈值→HALF_OPEN→30s后探测请求→成功回NORMAL/失败回HEAVY。解决"恢复时流量冲击"问题 |
| 🔀 **P0 多级优先级队列** | 3 级队列常量(REALTIME=10/NORMAL=5/BATCH=0)。`PriorityBlockingQueue` 按优先级+入队时间排序。`StreamChatController` 接收 priority 参数 |
| 🧠 **P1 小模型前后置处理** | `LightweightPrePostProcessor` 小→大→小三段流水线。前置用小模型验证意图匹配，后置用小模型校验回复质量(0-1评分)。异常不影响主流程 |
| 🎯 **P1 灰度发布调度** | `tier.canary-ratio` 按比例(0.0~1.0)放行新模型。`isCanaryRequest()` 基于 query hashCode 确定性灰度。同模型渐近切流, 对比指标后全量上线 |
| 📊 **P2 执行 DAG 可视化** | `GET /api/admin/execution-dag/{requestId}/view` → 内联 HTML 可视化页面。基于 Redis SSE 事件构建节点状态链(绿/蓝/红/黄/紫)，支持浏览器直接查看 |
| 📊 **P2 完成率看板(9面板)** | `a2a-daily-ops.json` 6→9面板。新增：完成率按Agent分组、档位选择分布、Agent平均迭代次数 |
| 🗂️ **P0 索引版本化 + 对账** | `KnowledgeDocument.indexVersion` 字段 + `IndexReconciliationService` 对账服务。索引策略/模型变化时递增版本，检索只查 active_index_version |
| 📐 **G5 KB 切块尺寸合规** | `ChunkStrategy.defaultMaxTokens()` 512→1024，`defaultOverlap()` 50→128，对齐文章 800-1200 中文汉字/chunk 推荐值 |
| 🔍 **P1 LoopGuardService 确定性快速判断** | Agent 输出后立即纯代码检测三类状态：阻塞标记(blocked/无法继续/需要授权)、用户决策请求(请选择/是否继续)、基础设施错误(timeout/500/rate limit)。11组Pattern覆盖80%异常场景，命中即暂停无需LLM评估 |
| 🔬 **P0 Cross-Encoder BgeReranker 精排** | 标题加权精排公式 `titleCos×0.4 + contentCos×0.6`。对 (query, doc) 对分别计算标题和全文语义相似度，综合评分重排。替代 `Reranker.identity()` 兜底 |
| 🏗️ **P0 Router 高可用多副本** | Nginx 负载均衡(Router×3 + Consumer×2)。Router 无状态(全部状态存入 Redis)，任意实例宕机不影响服务。docker-compose.ha.yml 一键部署 |
| 🚦 **P0 数据/控制平面分离** | `ControlPlaneEventBus` 专用通道(key 前缀 `ctrl:events:`)，隔离熔断/降级/告警等控制信令与业务消息。DegradationService 降级级别变更时自动发布控制平面事件 |
| 🔁 **P1 Handoff 节点级熔断** | `executeHandoffNode` 独立 `breakerFailureCounts` 计数器，连续失败≥2次跳过该 Agent。成功/失败均记录到 `DegradationService`，加入异常率滑动窗口统计 |
| 🎯 **意图定义向量化检索** | 5 个意图定义(ORDER/PRODUCT/GENERAL/COMPLEX/UNKNOWN)由 BGE 向量动态检索 Top-3，替代全量硬编码。BGE cosine 相似度(主)+关键词命中加权(辅)，不可用时降级纯关键词匹配 |
| ✅ **经验验证+淘汰机制** | ExperienceValidator 对召回经验验证时效性(>24h未命中标记过时)、可靠度(hitCount<3标记低置信)、新鲜度(未经验证)。ExperiencePruner 每日定时扫描淘汰低质量经验 |
| 🔀 **Handoff 显式交接** | Agent 执行完毕后可通过 `HandoffCommand(targetAgent, question, contextPayload)` 显式移交下一个 Agent，支持递归 Handoff 链。适用于客服转接等串行场景 |
| 🧠 **Agent 独立记忆** | Order/Product/General 各 Agent 拥有用户粒度的独立记忆文件(`{agent}-memory.md`)，通过 `savePreference`/`recallMemories` 工具存取偏好。Markdown 格式，与 Consumer 记忆系统一致 |
| ⏰ **记忆老化警告+条目截断** | 超过 7 天记忆标记 ⚠️、超过 30 天 ⚠️⚠️；超过 10 条时输出 key-only 索引并提示"可调用 recallMemories 获取详情"。Agent prompt 统一增加验证指令 |
| 🕵️ **后台自动提取偏好** | 每轮对话结束后，MemoryExtractor 异步扫描用户输入+Agent 回复，用 LLM 自动提取可复用偏好写入记忆。不限 Agent 手动调用 |
| 🔗 **Span 级嵌套追踪** | TraceSpan 基于 Micrometer Observation 为 LLM 调用(`agent-llm-call`)和工具执行(`agent-tool-execute`)创建 Jaeger 嵌套跨度，与现有 0.3 采样率配合 |
| 👁️ **P0 Langfuse LLM 可观测性集成** | 全模块 Brave→OpenTelemetry 迁移。`ChatModelCompletionContentObservationFilter` 将 Spring AI 每次 ChatModel 调用的 prompt/completion 全文 + token 用量写入 OTel span。设置 `OTEL_EXPORTER_OTLP_ENDPOINT` + `OTEL_EXPORTER_OTLP_HEADERS` 后自动上报 Langfuse，Jaeger 同步兼容 OTLP。零代码侵入 |
| 📝 **9 段式 Context 压缩** | 9 个结构化章节摘要(诉求/技术/错误/待办等)；增量更新避免重复压缩；MicroCompact 清理旧工具结果；PrecomputedCompact 后台异步预压缩 |
| 🔍 **上下文感知记忆排序** | `getAllFormatted(agent, userId, context)` 按问题关键词匹配排序记忆条目，最相关偏好优先展示 |
| 🔍 **Milvus 向量数据库** | 替代 pgvector，Milvus 集群(etcd+MinIO+Milvus)提供知识库持久化 + 经验向量检索(IVF_FLAT + COSINE)。Milvus 不可用时自动降级 pgvector。SHA-256 截断 64 位哈希替代 hashCode 避免 ID 冲突 |
| 🔒 **向量库 6 类生产字段** | 参考字节面试考点：Milvus/InMemory/PgVector 三个知识库全部补齐 ACL(tenant_id)、版本号(version)、来源(source_url)、段落索引(chunk_index)、更新时间(updated_at)、content_hash。每类字段对应一种生产事故，ACL 检索前过滤强制执行 |
| 💾 **对话摘要持久化到 Milvus** | 方案 A：每次 Context 压缩生成的 9 段式摘要自动嵌入为 BGE 向量，持久化到 Milvus `conversation_summaries` Collection。支持语义检索历史对话摘要 |
| 🧩 **System Prompt 分层** | PromptBuilder 三层组装: base-prompt(通用规则) + 服务自有指令 + 运行动态上下文 |
| 🎭 **P0 情感分析三重检测** | `SentimentAnalysisService` 5级情感(正面→愤怒) + 关键词库(90+条) + BGE 语义种子向量(cosine≥0.35) + LLM 三级兜底。`ChatConsumerService` 集成：等级≥4自动转人工，≥3注入 `[用户情绪:X(等级N)]` 前缀驱动下游语气调整。跨轮情绪追踪预警(连续3轮≥3触发) |
| ❤️ **P1 BGE 语义情感分析** | `BgeSentimentAnalyzer` 各等级预置10+种子文本→BGE向量缓存。处理"语义相似但字面不同"的表达(如"效率低下"→负面)，与关键词匹配互补 |
| 📝 **P0 工具输出截断（保头保尾砍中间）** | `SmartReActAgent` 中 truncateToolResult: 单次输出上限10K字符，头30%+标记+尾30%。防止100K token的工具输出撑爆上下文。只截输出不截输入 |
| 📎 **P0 引用绑定 [CID:doc-id]** | `KnowledgeHit.toContext()` 输出 `[CID:id] [版本:v2]` 稳定引用 ID。LLM 可在答案中引用 CID 实现结论→来源回溯。`ContextFaithfulnessChecker` 校验答案 CID 真实存在性 |
| 🔄 **P1 双阈值上下文压缩** | SmartReActAgent Scoped(净增90%) + FullWindow(总量95%) 双阈值。`prefillBaseline` 压缩后更新基线，只计净增长。`CompressionHooks` 接口 Pre/Post 回调 |
| 🛡️ **P1 Prompt 注入防御** | `AgentSafetyService` 关键词+正则双检：忽略规则、重写提示、泄露系统指令、假装完成、越权指令5类模式。`filterSafe()` 批量过滤检索结果 |
| 📊 **P1 Token Budget Reminder** | Token 达窗口75%阈值时注入提醒，每窗口仅一次。Agent 可据此收尾、简化或请求压缩 |
| ⚠️ **P1 上下文冲突标记** | `ContextFaithfulnessChecker.markConflicts()` 检测正反义词冲突(是/否、有效/无效等4组)，在上下文首部添加冲突警告 |
| 🛡️ **P0 内容安全护栏（SafeGuardAdvisor）** | `AdvisorChainAutoConfiguration` 注册 `SafeGuardAdvisor`(Order=50，前置所有 Advisor)，复用 `AgentSafetyService.detectInjection()` 检测 Prompt 注入；命中抛 `PromptInjectionBlockedException` 并发布 `BLOCKED` 审计事件。`SmartReActAgent` catch 分支友好降级。默认开(`safe-guard.enabled`) |
| 📊 **P0 调用审计闭环（AiAuditEvent）** | `TokenUsageAdvisor` 重写：真实从 `chatResponse.metadata.usage` 抽 prompt/completion/total token + 推断 provider/model；发布结构化 `AiAuditEvent`(traceId/tenantId/provider/model/tokens/latencyMs/resultType) 落内存环形缓冲 `AiAuditStore` + 结构化日志。`resultType` ∈ {SUCCESS,ERROR,BLOCKED} |
| 🏭 **P1 统一 ChatClient 工厂（AiChatService）** | `AiChatService` 收口全部 Advisor 装配：`buildChatClient(ChatModel)` / `applyAdvisors(Builder)`，按 null 装配 SafeGuard+TokenUsage+ThinkingCollector+PromptAudit。Order/General/Product AgentConfig + Consumer 4 服务 + Router 5 服务全部消除 `defaultAdvisors` 样板，统一接入审计/护栏链 |
| 🔎 **P1 @Tool 自动发现（AiToolRegistry）** | `AiToolRegistry.assemble(Object...)` 用 `MethodToolCallbackProvider` 聚合所有 `@Tool` 对象，跳过 null；四模块工具注册样板收敛为单行 `assemble(orderTools, analyticsTool, ...)` |
| 💾 **P2 类型安全 ChatMemory（RedisChatMemory）** | `common.memory` 抽象层：`ChatMemory` 接口 + `InMemoryChatMemory`(有界环形) + `RedisChatMemory`(StringRedisTemplate 存 `a2a:chat:memory:{convId}`，有界+TTL)。`MessageCodec` 用显式 `type` 信封规避 `Message` 多态反序列化损坏，`USE_LONG_FOR_INTS` 修 metadata 数字窄化。多实例部署可切 Redis 实现 |
| 🗜️ **P2 实体化结构化输出（entity()）** | `AiChatService.entity(ChatModel, sys, user, Class<T>)` 封装 Spring AI 结构化输出，替换脆弱的 `.call().content()` + 文本 JSON 解析。`OrderIntentService`/`LLMPreferenceExtractor` 改用 record + `entity()` 绑定，删除手工 `readTree` 映射 |
| 🏛️ **RAG 七连问工程化闭环** | 数据源→清洗→PDF解析→知识抽取→增量更新→冲突处理→版本治理全链路落地：`DocumentParseRouter`(多格式路由) + `KnowledgeIngestionService`(变更检测+多源分流+实体抽取) + `CrossDocumentConflictResolver`(Q6跨文档冲突) + 非覆盖式版本治理(`DocumentStatus`+`AuthorityLevel`)。详见「知识库召回管道」分节 |
| ⚖️ **Q6 检索侧跨文档冲突消解** | `CrossDocumentConflictResolver` 在 rerank 后对**跨文档**冲突(同结论相反表述)做权威裁决：先按 `AuthorityLevel`(L1官方>L2手册>L3笔记>L4用户)，同级按 `version`(高版优先)，败方相对扣分(`relativePenalty`)。`InMemory`/`PgVector`/`Milvus` 三个知识库 `search()` 末尾统一调用，返回 `ScoreBreakdown` 写入 `RetrievalTrace` 可观测 |
| 📑 **非覆盖式版本治理 (Q5/Q7)** | `DocumentStatus`(ACTIVE/SUPERSEDED/QUARANTINED) + `AuthorityLevel`(L1-L4) 取代简单覆盖更新：新版入库标记旧版 SUPERSEDED 而非物理删除，`QUARANTINED` 隔离低质量来源。`CacheVersionManager` 全局版本戳驱动语义缓存失效(`cacheVersionAtSave` 写入+读取校验)，知识库更新后 `fireCacheInvalidationHooks()` 自动失效旧缓存 |
| 🔀 **多源分流摄入** | `KnowledgeIngestionService` Step 3 按 `contentType` 分流：结构化 PDF 表格 / HTML / Word 绕过通用 chunker 直接 `new KnowledgeDocument` 入库，保留表格行列结构；纯文本/Markdown 走语义+递归分块。避免结构化内容被切碎丢失语义 |
| 🔤 **OCR 引擎接入** | 可插拔 `OcrStrategy` 接口 + `NoopOcrStrategy`(默认降级) + `TesseractOcrStrategy`(`ProcessBuilder` 调系统 `tesseract` CLI，零新增 Maven 依赖)。`PdfDocumentParser` 默认 `OcrStrategies.autoDetect()`：部署装好 Tesseract 后自动对文字层缺失的 PDF 页做 OCR 补录，未安装则零开销降级 |
| 🕸️ **实体关系 LLM 抽取 + 通用知识图谱** | `EntityExtractor`(可插拔) + `LlmEntityExtractor`(复用 `AiChatService.entity()` 结构化输出抽取实体/关系) + `KnowledgeGraphService`(内存实现+PG邻接表)。`KnowledgeIngestionService` 摄入时联动抽取，跨文档同名实体按 `ent:`+名称哈希稳定合并。`RagGraphAutoConfiguration` 在持有 `ChatModel` 的应用自动装配 Bean |
| 🖼️ **多模态 RAG 入库** | 新增 `common.rag.multimodal` 包：`ImageCaptioner`/`NoopImageCaptioner` + `OllamaVisionImageCaptioner`(Spring AI 2.0 多模态 `ChatClient.media(Media)`，零新依赖) + `MultimodalIngestor`(图片→视觉描述→`KnowledgeDocument`，docId 由字节 SHA-256 派生保证幂等)。`RagMultimodalAutoConfiguration` 按 `rag.multimodal.vision.enabled` 装配，默认 Noop 降级。`KnowledgeIngestionService.ingestImages()` 联动摄入 |
| 🚪 **评测闭环门禁 (CI)** | `EvalGateConfig`+`EvalBaseline`+`EvalGate`+`EvalReportExporter` 四件套 + `GoldenSuiteEvalGate` 编排器，双保险机制：**绝对阈值**(防断崖) + **基线回归**(±0.05 防渐变)。**已接入 EvalPipeline**：注入 `TrialExecutor` 且 `enableAgentGate && agentTrialCount>1` 时，Agent 用例走 Trial×pass^k 管线(`EvalPipeline`)，以 **pass^k ≥ minAgentPassKRate**(默认0.8) 作为单用例稳定性门禁依据，并联动 Grader 瀑布流/人工路由/根因分析；未注入运行器时保持离线"待执行"占位，向后兼容。`.github/workflows/eval-gate.yml` 在 push/PR 到 main 时自动 compile-all + 跑黄金集评测，首次自举生成 `eval-baseline.json`，门禁不通过则 CI 失败阻断合入 |

---

## 系统架构

```text
┌─────────────┐     ┌──────────────┐     ┌───────────────────────────┐
│   Frontend  │────▶│   Gateway    │────▶│        Router             │
│  React:3001 │     │  :8081 (JWT) │     │  :8083 (意图识别           │
└─────────────┘     └──────────────┘     │  + 任务分析                │
                                          │  + 评测后处理              │
                                          │  + 查询改写                │
                                          │  + DAG 协作               │
                                          │  + 关键词快车道            │
                                          │  + 阶段感知 Prompt)        │
                                          └──────────┬────────────────┘
                                                   │
                          ┌─────────────────────────┼──────────────┐
                          │                         │              │
                    ┌─────▼──────┐          ┌───────▼──────┐      │
                    │  Consumer  │          │   General    │      │
                    │  :8082     │          │   :8087      │      │
                    │ (会话管理   │          │  (闲聊+天气   │      │
                    │  记忆沉淀)  │          │   换算工具)   │      │
                    └─────┬──────┘          └──────────────┘      │
                          │                                        │
               ┌──────────┼──────────┐                             │
          ┌────▼────┐ ┌──▼────┐ ┌───▼────┐   ┌───────────────┐    │
          │  Order  │ │Product│ │  User  │   │  Recommend    │    │
          │  :8085  │ │:8084  │ │  :8086 │   │  :8088 (P3)   │    │
          │(订单客服)│ │(商品咨询)│ │(认证)   │   │(图谱+协同过滤)│    │
          │(含知识库)│ │(含知识库)│ └────────┘   └───────────────┘    │
          │(含图谱) │ │(含图谱) │                                     │
          └─────────┘ └───────┘                                     │
                                                                     │
                    ┌───────────────────────────────────────────┐    │
                    │         Infrastructure                     │    │
                    │  Redis ─ Nacos ─ PostgreSQL                │    │
                    │  Milvus(etcd+MinIO) ─ Prometheus ─ Grafana │    │
                    │  Loki ─ Jaeger ─ Embedding(:8091)          │    │
                    └───────────────────────────────────────────┘────┘
```

### 请求流程

1. **前端请求** → Gateway (JWT 认证 + 限流)
2. **Gateway 转发** → Router (多 Agent 协作路由)
3. **Router 路由** → Consumer (会话管理 + 价值评估 + 记忆沉淀)
4. **Consumer 调度** → 对应 Agent(s)（通过 Nacos 服务发现）
5. **Agent 响应** → 通过 SSE 流式返回给前端
6. **价值评估** → 轮数≥3 或触发工具调用时触发 → `data/users/{userId}/memories/`（异步增量追加）

### 多 Agent 协作

复杂问题（如"推荐北京景点和川菜"）自动分解为子任务，顺序执行带共享上下文：

```
用户: "周末去北京玩，推荐景点和川菜馆"
  │
  Router.executeCollaborative()
  ├─ TaskPlannerService.plan()
  │   ├─ AgentDiscoveryService.getCachedAgents()  ← 动态发现
  │   └─ LLM 分解:
  │      t1|北京热门景点|location_weather
  │      t2|北京川菜餐厅|food_recommendation
  │
  ├─ parallelExecute() [带共享上下文]
  │   ├─ callAgent("location_weather", "北京景点")
  │   │   └─ 结果: 故宫、天坛、颐和园...
  │   │   └─ 存入 sharedContext
  │   │
  │   ├─ callAgent("food_recommendation",
  │   │     "北京川菜" + sharedContext)     ← 注入前序结果
  │   │   └─ 结果: 眉州东坡(近故宫)、...
  │   │   └─ 存入 sharedContext
  │   │
  │   └─ (若有更多维度以此类推)
  │
  ├─ ResultMerger.merge()
  │   └─ LLM 整合: "游览故宫后可去眉州东坡用餐..."
  │
  └─ 全部失败 → inlineFallback() ← 内联 ChatClient 终极兜底
```

**共享上下文**：子任务顺序执行，后执行的 Agent 能看到前面 Agent 的输出（如美食 Agent 推荐故宫附近的餐厅），无需 Agent 端改造。

---

## 技术栈

### 后端

| 分类 | 技术 | 版本 |
|------|------|------|
| 语言 | Java | 21+ (Temurin 21.0.6 LTS, `D:\Program Files\Java\jdk-21.0.6+7`) |
| 框架 | Spring Boot | 3.4.8 |
| AI 框架 | Spring AI（社区版） | 2.0.0 |
| 模型 | **DeepSeek V4-Flash**（通用/订单/商品 Agent 主通道）+ **Ollama 本地推理**（deepseek-r1:7b Router 路由 + qwen2.5:3b 轻量任务） + BGE-large-zh ONNX (384d RAG) / BGE-small-zh ONNX (512d Cache) / bge-reranker-v2-m3 ONNX (重排序，实验性) | — |
| 注册/配置中心 | Nacos（元数据支持动态管理） | 2.3+ |
| 缓存 | Redis | 7.2+ |
| 数据库 | PostgreSQL (向量: Milvus) | 16+ (Milvus 2.5+) |
| 向量数据库 | Milvus（etcd+MinIO），IVF_FLAT + COSINE 索引 | 2.5.5 |
| 文档解析 | Apache PDFBox + Apache POI + JSoup | 3.0.3 / 5.3.0 / 1.18.3 |
| ORM | MyBatis-Plus | 3.5+ |
| 分词 | HanLP | 1.8.4 |
| SQL 解析 | jsqlparser | 4.9 |
| 推理引擎 | ONNX Runtime | 1.20.0 |
| **Spring AI Advisor** | spring-ai-client-chat（Token 采集/思维链/审计） | **2.0.0** |
| **数据库方言** | PostgresDialect / MySQLDialect（12 个方言方法，自动切换） | **内置** |
| **SSE 框架** | SseEvent + SseEventBus（Redis 缓存/断线续传/代理转发） | **内置** |
| **安全** | UrlSecurityValidator（SSRF 防护） + SqlSecurityValidator（AST SQL注入防护） | **内置** |
| 构建 | Maven Wrapper | 3.9.6 |

### 前端

| 分类 | 技术 |
|------|------|
| 框架 | React 18 + TypeScript |
| UI 库 | TDesign |
| 通信 | WebSocket / SSE |
| 构建 | Vite |

### 监控

| 工具 | 用途 | 端口 |
|------|------|------|
| Prometheus | 指标收集 | 9090 |
| Grafana | 可视化仪表盘 | 3000 |
| Jaeger | 链路追踪 | 16686 |
| Loki | 日志聚合 | 3100 |
| Promtail | 日志采集 | — |
| Zipkin | 追踪兼容层 | 9411 |

### 推理引擎

系统采用 **双推理通道设计**，通过 Spring AI 原生 `ChatModel` 接口统一调用：

| 通道 | 模型 | 用途 | 调用方式 |
|------|------|------|---------|
| 🚀 **DeepSeek 云端** | deepseek-v4-flash | General/Order/Product Agent 主推理 | Spring AI 自动配置 `deepSeekChatModel` |
| 🏠 **Ollama 本地** | qwen2.5:7b | Router 路由决策、Consumer/Order 兜底降级 | Spring AI 自动配置 `ollamaChatModel` |
| ⚡ **轻量通道** | qwen2.5:3b | 对话摘要、偏好提取、缓存改写、关键词提取 | 委托模式 `lightChatModel` |

**2026-06-29 重构要点**：
- 全面启用 **Spring AI 原生集成**，移除自定义 HTTP 客户端
- 所有 DeepSeek/Ollama 调用走 Spring AI 自动配置，`ChatModel.stream()` 返回真实 SSE
- 轻量通道采用委托模式注入 auto-configured `OllamaChatModel`，避免重复构造复杂依赖

> 💡 DeepSeek 云端推理需要配置 `DEEPSEEK_API_KEY` 环境变量。

---

## 知识库召回管道

系统在 **Order** 和 **Product** 模块实现了基于 BGE 向量检索的知识库召回管道，用于从订单政策/商品信息知识库中检索相关内容增强 Agent 回答。

### 文档预处理管道

```
源文档 (PDF / Word / HTML / TXT)
    │
    ├── DocumentParseRouter（按扩展名自动路由）
    │   ├─ .pdf  → PdfDocumentParser（PDFBox 3.x，按页+段落解析）
    │   ├─ .docx → WordDocumentParser（Apache POI，按标题/段落/表格）
    │   ├─ .html → HtmlDocumentParser（JSoup，去噪+按h1~h6分节）
    │   └─ .txt  → TextFallbackParser（按空行分段）
    │
    ├── ParsedDocument（14 字段：content/sourceUrl/pageNumber/section/
    │   contentType/tenantId/version/effectiveAt/expireAt/contentHash/...）
    │
    ├── DocumentChunker（编排器）
    │   ├─ SemanticChunkStrategy（按语义边界分块：MD标题/章节编号）
    │   └─ RecursiveChunkStrategy（超长段落fallback，支持overlap）
    │
    └── KnowledgeDocument → 向量入库 + BM25索引
```

### 变更检测（2026-07-02 新增）

```
KnowledgeIngestionService.parseAndIngest(filePath, tenantId)
    │
    ├── Step 1: DocumentParseRouter.parse(filePath)
    │            → List<ParsedDocument> (含 contentHash)
    │
    ├── Step 1.5: ContentHashCache 变更检测
    │   ├── 按 baseDocId 分组（getBaseDocId 去除版本后缀）
    │   ├── HashUtil.aggregateHash(contents)
    │   │    对各段落做 normalize_text()（去时间戳/页脚页码/控制字符/连续空白）
    │   │    再拼接后计算 SHA-256
    │   └── ContentHashCache.needsReingest(baseDocId, newHash)
    │        ├── 首次摄入 → 标记为「已变更」，正常处理
    │        ├── hash 匹配 → 返回 IngestionResult.skipped（跳过全链路）
    │        └── hash 不匹配 → 标记为「已变更」，正常处理
    │
    ├── Steps 2~6: 注入租户 → 分块 → 入库 → reindex
    │
    └── Step 7: updateHashCache() 更新缓存哈希
```

**变更检测效果：**

| 场景 | 处理 | 节省流程 |
|:----|:-----|:---------|
| 首次摄入 | 全量处理，更新缓存 | — |
| 相同文档再次摄入 | hash 匹配 → 跳过 | 跳过解析/分块/嵌入/入库/reindex |
| 文档内容更新后摄入 | hash 不匹配 → 完整处理 | 正常流程 |
| 时间戳假变更（"更新时间：2026-06-29"） | normalize_text() 过滤后 hash 不变 → 跳过 | 避免误判全量重建 |

**工具类：**

- `HashUtil` — `normalizeText()` 去除假变更源（更新时间行、日期时间行、页脚页码、控制字符、连续空白），`sha256Hex()` 计算 SHA-256，`aggregateHash()` 多段落聚合
- `ContentHashCache` — ConcurrentHashMap 实现，支持 `hasChanged()` / `needsReingest()` / `remove()`，可替换为 Redis 实现

### Pipeline RAG 检索架构

```
用户查询
    │
    ├── Pipeline Handler 链 (按 Order 编排)
    │   ├── H00 MultiQueryHandler        (Order=0)   查询扩展(3角度变体)
    │   ├── H01 QueryRewriteHandler      (Order=2)   LLM 查询改写
    │   ├── H02 ExactMatchHandler        (Order=10)  精确匹配
    │   ├── H03 KeywordSearchHandler     (Order=20)  关键词搜索
    │   ├── H04 Bm25SearchHandler        (Order=30)  BM25 评分
    │   ├── H05 KnowledgeSearchHandler   (Order=40)  知识库检索
│   ├── H06 GraphSearchHandler       (Order=50)  图谱检索(商品关系推荐)
│   ├── H06b LightRagSearchHandler   (Order=60)  实体关系图检索(LightRAG 查询侧)
│   ├── H99 RrfFusionHandler         (Order=100) RRF 融合 + 质量评估
    │   ├── H98 DedupHandler             (Order=105) 内容去重(SHA-256 + Jaccard)
    │   └── H97 RerankHandler            (Order=110) 嵌入精排(余弦相似度)
    │
    ├── 可插拔 SPI：实现 RagSearchHandler 接口 + @Component 即可加入
    ├── 质量门禁 qualityScore ≥ 0.30 → 正常返回，< 0.30 → INSUFFICIENT_EVIDENCE
```

### 版本控制

```
KnowledgeDocument.isSupersededBy(newerDoc):
    同 baseDocId（如 "ORD-REFUND-001"）且版本更高时标记被取代

reindex() 时自动执行 removeSuperseded()：
    按 baseDocId 分组 → 保留最高版 → 清除旧版数据
    无需手动管理，添加新版本文档后 reindex 即自动清理
```

### ACL 检索前过滤

```
所有 KnowledgeBase.search(query, topK, tenantId) 强制 ACL（租户隔离，基础层）：

InMemoryKnowledgeBase:  search 循环中 tenantMatches() 过滤
PgVectorKnowledgeBase:  SQL WHERE 子句 buildAclClause()
MilvusKnowledgeBase:    search 参数 withExpr(buildAclExpr())

原则：权限过滤在检索前完成（返回前过滤），
     先召回高权限内容再让模型"不要说出来" = 自欺欺人
```

#### 细粒度 ACL（P3-A，对标文章⑤：权限进入检索层，服务端生成 filter）

在租户隔离之上补齐三个维度，filter **完全由服务端根据请求身份生成**，绝不信任客户端传入条件：

| 维度 | 字段 | 语义 |
|:----|:-----|:-----|
| 角色 | `authorizedRoles` ⊆ `AclContext.roles` | 文档为空 = 租户内任意角色可见；否则需命中其一 |
| 用户 | `authorizedUsers` ⊇ `AclContext.userId` | 文档为空 = 租户内任意用户可见；否则需精确命中 |
| 安全等级 | `securityLevel` ≤ `AclContext.securityClearance` | 用户许可等级必须 ≥ 文档密级方可查看 |

实现要点：
- 新增 `AclContext` 值对象（`tenantId` + `userId` + `Set<roles>` + `securityClearance`），工厂 `forTenant` / `publicContext` / `fromMdc` / `builder`；`fromMdc()` 从请求 MDC 读取身份并退化为公开上下文（向后兼容）。
- `KnowledgeDocument` 新增 `authorizedRoles` / `authorizedUsers` / `securityLevel` 三字段。
- 四种 `KnowledgeBase` 全部实现 `search(query, topK, AclContext)`：PgVector 走 `buildAclClause()` 四层 SQL；Milvus 走 `buildAclExpr()`（`array_contains_any` / `array_contains` / 数值比较）；InMemory 走 `tenantMatches()` 四层过滤；Tiered 双冷/热层均应用。
- 检索链路贯通：`KnowledgeRetrievalService.search(..., AclContext)` + `KnowledgeQueryTool` 注入 `AclContext.fromMdc()`，将请求身份透传到检索层。
- `KnowledgeBase` 接口保留 default `search(query, topK, tenantId)` 委托 `search(query, topK, AclContext.forTenant(tenantId))` 保证向后兼容。

### 知识库内容

| 知识库 | 文档数 | 覆盖内容 |
|:------|:-----:|:---------|
| **订单知识库** (order_knowledge) | 10 | 退款政策、发货规则、支付说明、订单状态、优惠券规则、客服联系、售后服务、预约规则等 |
| **商品知识库** (product_knowledge) | 7 | 退换货政策、保修政策、价格保护、配送说明、库存规则等 |

### 存储方案

| 模式 | 说明 | 配置 |
|:----|:-----|:-----|
| **InMemory**（默认） | 内存索引 + BGE 向量 + BM25，启动时加载种子数据 | `app.milvus.enabled=false` |
| **Milvus**（可选） | Milvus 集群持久化(IVF_FLAT + COSINE)，跨实例共享 | `app.milvus.enabled=true`，需先 `docker compose -f monitoring/docker-compose-milvus.yml up -d` |

### 非覆盖式版本治理（Q5 / Q7）

取代「简单覆盖更新」，新版文档入库时**标记旧版为 SUPERSEDED 而非物理删除**，支持审计回溯与灰度回滚：

```
KnowledgeDocument
  ├── DocumentStatus: ACTIVE / SUPERSEDED / QUARANTINED
  │     └── QUARANTINED 隔离低质量/未验证来源，不参与检索
  └── AuthorityLevel: L1_OFFICIAL > L2_MANUAL > L3_NOTE > L4_USER
        └── 跨文档冲突裁决与缓存失效优先级依据

CacheVersionManager（全局版本戳）
  ├── 知识库更新 → bumpVersion() 全局 +1
  ├── 语义缓存写入记录 cacheVersionAtSave（CachedRouteDecision.cacheVersionAtSave）
  ├── 读取时 isVersionValid(cacheVersionAtSave) 校验，失效旧缓存
  └── 知识库更新后 fireCacheInvalidationHooks() 自动触发缓存失效
```

### 检索侧跨文档冲突消解（Q6 第二层）

`ContextFaithfulnessChecker`(markConflicts) 处理**上下文内**正反义词冲突；`CrossDocumentConflictResolver` 处理 **rerank 后跨文档**的相反结论冲突（如 L1 官方说"支持"、L4 用户笔记说"不支持"）：

```
InMemory / PgVector / Milvus 的 search() 末尾统一：
  hits = conflictResolver.resolveHits(hits, query)
    ├── 按 (归一化结论, 冲突组) 聚类相反表述
    ├── 裁决：AuthorityLevel 高者胜 → 同级按 version 高者胜
    ├── 败方 relativePenalty 相对扣分（不归零，保留弱信号）
    └── ScoreBreakdown 写入 RetrievalTrace（reason=CONFLICT_PENALTY）
```

`OrderKnowledgeConfig` 经 `ObjectProvider<CrossDocumentConflictResolver>` 自动挂接到 InMemory 种子库与 Milvus 实例。

### 多源分流摄入

`KnowledgeIngestionService.parseAndIngest` Step 3 按 `contentType` 分流，避免结构化内容被通用 chunker 切碎丢失语义：

| contentType | 处理 | 说明 |
|:-----------|:-----|:-----|
| `pdf-table` / `html` / `word` | 绕过 chunker，直接 `new KnowledgeDocument` 入库 | 保留表格行列/标题层级结构 |
| `text` / `markdown` | 语义分块 + 递归分块（支持 overlap） | 通用长文本路径 |

### OCR 引擎接入

可插拔 `OcrStrategy` 接口，PDF 文字层缺失时补录：

```
OcrStrategy (接口)
  ├── NoopOcrStrategy      （默认降级，isAvailable=false，零开销）
  └── TesseractOcrStrategy （ProcessBuilder 调系统 tesseract CLI，零新增 Maven 依赖）
        └── tesseract --version 探测可用性；extractText 写临时 PNG → OCR → 读结果，异常隔离 + 临时文件清理
             默认语言 eng（中文部署注入 chi_sim+eng）

PdfDocumentParser.ocrStrategy = OcrStrategies.autoDetect(lang)
  └── Tesseract 可用 → 真实 OCR；不可用 → 自动降级 Noop
```

**激活**：Docker 镜像/主机安装 `tesseract-ocr`（中文加 `tesseract-ocr-chi-sim`），无需改代码。

### 实体关系 LLM 抽取 + 通用知识图谱

与 P3 `ProductGraphService`(硬编码商品图谱) 互补，这里提供**通用**实体/关系抽取框架，对任意入库文档生效：

```
EntityExtractor (接口, 可插拔)
  ├── NoopEntityExtractor   （默认降级）
  └── LlmEntityExtractor    （复用 AiChatService.entity() 结构化输出抽取 EntityNode + EntityRelation）
        └── 实体 ID = "ent:" + 名称归一化 SHA-256 → 跨文档同名实体稳定合并
             超长文本截断 6000 字；异常隔离返回空，不中断摄取

KnowledgeGraphService（内存实现 + PG 邻接表 DDL）
  └── KnowledgeIngestionService 摄入时联动抽取（图谱服务为 null 或抽取器不可用则空操作）

RagGraphAutoConfiguration：@ConditionalOnBean(ChatModel.class) + @ConditionalOnMissingBean
  └── 任意持有 ChatModel 的 Spring Boot 应用自动获得 KnowledgeGraphService Bean
```

#### LightRAG 实体关系图检索（查询侧，2026-07-09 落地）

通用知识图谱建好后，需在**查询阶段**把它接入检索链路，否则图只是"建而不用"。新增 `LightRagSearchHandler`（实现 `RagSearchHandler`，Order=60，位于 `GraphSearchHandler` 与 `RrfFusionHandler` 之间），作为一条独立召回路径并入 RRF 融合：

```
查询 query
  → 抽取消歧 token（整句 + 中英文分词 + 中文长串二元滑动切分，提升召回）
  → KnowledgeGraphService.searchNodes(token) 匹配实体（名称/类型/描述子串）
  → 以匹配实体为起点 getRelationsForNode() 扩展 1 跳关系
  → 组装「实体A —关系— 实体B」三元组（置信度降序，起点≤6 / 三元组≤15）
  → context.addPathResult("实体关系图检索", items)  → 由 RrfFusionHandler 自动并入最终答案
```

与 `GraphSearchHandler`(H06，仅基于写死的商品编码关系做推荐) 的区别：本 Handler 消费的是 **LLM 从文档抽取出的通用实体/关系图**，可回答「A 和 B 什么关系」「为什么 X 影响 Y」这类多跳/关系型问题。

**图谱激活（让图在运行时真正有数据）**：`ProductKnowledgeConfig` 新增 `CommandLineRunner`，启动后将种子文档（产品/订单）经 `KnowledgeGraphService.extractFromDocument()` 抽取进图——异步、best-effort，Ollama 未就绪时由 `KnowledgeGraphService` 内部 try/catch 兜底，不影响启动；图谱 Bean 缺失则整体跳过。

**优雅降级**：`KnowledgeGraphService` 注入 `required=false`；图为空 / Bean 缺失 / 检索异常时静默跳过，不影响其它召回路径。

**配置项**（`smart-assistant-product/src/main/resources/application.yml`）：

| 配置 | 默认值 | 说明 |
|:----|:----:|:-----|
| `product.rag.lightrag.enabled` | `true` | 查询侧「实体-关系图检索」Handler 开关 |
| `product.rag.lightrag.ingest-seed.enabled` | `true` | 启动时将种子文档抽取进知识图谱（异步 best-effort） |

### 多模态 RAG 入库

图片/截图经视觉模型转为文本后纳入知识库检索：

```
common.rag.multimodal
  ├── ImageCaptioner (接口) / NoopImageCaptioner (默认降级)
  ├── OllamaVisionImageCaptioner
  │     └── Spring AI 2.0 多模态：ChatClient.prompt().user(u -> u.media(Media...)).call()
  │        零新依赖；实际调用抽至 doCaption 便于单测桩接；异常隔离返回描述失败标记
  ├── ImageCaptioners.autoDetect(chatModel, enabled) 工厂
  └── MultimodalIngestor：图片 → 视觉描述 → KnowledgeDocument
        └── docId = "img:" + 字节 SHA-256（保证同图幂等，重摄入自动跳过）

RagMultimodalAutoConfiguration：rag.multimodal.vision.enabled=true 时装配视觉 captioner，否则 Noop
KnowledgeIngestionService.ingestImages(Paths, tenantId)：联动摄入，成功后触发缓存失效 + 索引重建
```

**激活**：配置视觉 `ChatModel` Bean + `rag.multimodal.vision.enabled=true`；未配置时零开销降级。

---

## Router 流水线

Router 的路由决策采用 **14 步流水线**（含 Step 0 多意图直通短路）：

```
用户请求
  │
  ├→ Step 0: 经验匹配 (ExperienceService) 
  │   BGE 向量匹配 TOOL/COMMON/REACT 经验，命中 TOOL 经验直接执行
  │   ⭐ 经验召回后经 ExperienceValidator 验证时效性/可靠度/新鲜度（警告随结果返回）
  │   ⭐ ExperiencePruner 每日 3:00 定时淘汰低质量经验（低 hitCount + 长空闲）
  │   ⭐⭐ 多意图检测：secondaryIntents 非空时直接并行调度多个 Agent
  │      虚拟线程并行调用 → CompletableFuture.allOf → 合并结果
  │      → 完全跳过 Step 1-5（无 LLM/缓存/上下文/改写）
  │
  ├→ Step 0.5: 关键词快车道 (KeywordFastRouteService) ⚡
  │   高频明确意图（退款/查订单/取消/问候/商品查询）跳过 LLM 分诊
  │   ⭐ 7.3μs/次，命中率 100%，加速比 ~68,000x
  │   ⭐ 跨 Agent 多意图检测：匹配到多条不同 Agent 规则时返回 null，走全管道
  │
  ├→ Step 1: 语义缓存 (SemanticCacheService)
  │   Tier 1 精确 → Tier 2 关键词 → Tier 3 BGE 向量; 命中直接返回
  │
  ├→ Step 2: L3 三路并行意图融合 (IntentFusionService) ⚡
  │   规则(KeywordFastRoute) ∥ 小模型(BGE余弦分类器) ∥ LLM(TaskAnalysis)
  │   ⭐ 规则≥0.9→直接输出(7μs)/小模型≥0.7→优先(10ms)/LLM兜底(200ms)
  │   ⭐ L5 意图漂移检测: 当前轮vs上轮用户问题BGE相似度<0.80触发
  │   融合结果经 IntentEvaluationService 规则层评测后处理
  │   实体归一化(日期/金额/地点/纠错) → 词槽分析(填充/缺失/冲突)
  │   → 澄清判断(追问生成) → 置信度阈值检查(<0.6触发澄清)
  │   存入 Redis 供下游消费
  │
  ├→ Step 3: 意图引导的查询改写 (IntentGuidedQueryRewriter) 
  │   根据意图类型选择改写策略：
  │   多意图→查询分解 / 模糊→查询扩展 / 精确→保留 / 对话→指代消解
  │   改写结果存入 Redis a2a:rewrite:{id}
  │
  ├→ Step 4: 构建上下文 + RAG 增强
  │   从 Redis 加载会话历史，可选 RAG 检索增强问题
  │   ⭐ 摘要回指：extractEntities() 提取六类实体(ORDER_ID/PRODUCT/AMOUNT/DATE/ACTION/DECISION)
  │   生成结构化 `【历史引用】` 文本，与 `【上下文】`+`【当前问题】` 三段式拼接
  │
  ├→ Step 5: 路由决策 (executeCollaborative)
  │   多 Agent DAG 协作: 图分解 → 拓扑并行执行 → Handoff 显式交接
  │   ⭐ ErrorType 错误分类：RETRYABLE→指数退避重试(1s→2s→4s, 最多3次)
  │   ⭐ successCriteria 验收：ReflectionService.checkCriteria() LLM 校验输出
  │   ⭐ NEED_REPLAN→TaskPlannerService.replan() 动态追加新节点
  │   ⭐ HandoffCommand(targetAgent, question, 累积上下文) 支持动态链式移交
  │   ⭐ 每个节点执行后可发起 Handoff，递归执行直到 COMPLETE/FAILED
  │   ⭐ 节点级熔断：同一 Agent 连续失败≥2次后跳过其后续节点
  │   ⭐ Handoff 节点级熔断：独立计数器，连续失败≥2次跳过
  │   ⭐ ⭐ Checkpoint 持久化：每轮执行后自动保存状态到 Redis，支持断点恢复
  │   ⭐ ⭐ 条件边(Conditional Edge)：运行时评估条件依赖(RESULT_CONTAINS/NOT_CONTAINS/SUCCESS/FAILED)
  │   ⭐ ⭐ Graph 循环(rerouteTarget)：条件满足时跳转前驱节点形成循环，maxGraphIterations 预算保护
  │   ⭐ ⭐ HITL 节点自动暂停并等待审批
  │   ⭐ ⭐ Handoff 链 Checkpoint 持久化
  │   ⭐ 异常率降级：DegradationService 滑动窗口监测，>20%切单Agent/>40%回退内联
  │   ⭐ Agent HTTP 调用熔断保护：Resilience4j CircuitBreaker，50%失败率→30s半开
  │   三级降级: Agent→inlineFallback→预设文案轮换
  │
  ├→ Step 5.5: 工具健康检查 (RoutingToolChecker) 🔑
  │   路由决策后检查目标 Agent 关键工具是否注册可用
  │   未通过时打印警告日志，配合 GET /tools/health 端点监控
  │
  └→ Step 6: 后处理 (finalizeRouting)
      ├─ Agent 执行事件发布 (AgentEventBus) 📋 → 持久化状态机事件链
      ├─ 反射器 (ReflectionService) — 纯规则五维评分
      ├─ LLM 质量评估 (QualityEvaluationService) — 四维语义评分
      ├─ 错误码恢复 (ErrorRecoveryService) — 表驱动重试
      ├─ 语义缓存写入 + 经验提取
      ├─ Bad Case 自动挖掘 (BadCaseMinerService) — 低置信度记录到 Redis
      └─ 完整决策写入 Redis (Consumer 通过 BLPOP 阻塞读取)
```

### 反射器（规则级，零 LLM 开销）

五维评分模型（总分 0.0 ~ 1.0，默认阈值 0.6）：

| 维度 | 权重 | 评分规则 | 说明 |
|------|:----:|----------|------|
| 长度检查 | 20% | ≥100字=1.0 / 50~99字=0.6 / 20~49字=0.3 / <20字=0.0 | 回复过短疑似异常 |
| 错误标记检测 | 25% | 不含=1.0 / 含❌⚠️ERROR等=0.0 | 明确的服务错误标记 |
| 关键词覆盖 | 25% | ≥50%=1.0 / 30%~50%=0.6 / <30%=0.3 | 问题中命名实体是否在回复出现 |
| Agent 健康状态 | 15% | 在线=1.0 / 离线=0.5 | Nacos 服务发现状态 |
| 意图匹配 | 15% | 匹配=1.0 / General=0.6 / 不匹配=0.3 | intentTag 与 Agent 对应关系 |

### 工作流程

```
Agent 返回结果
  │
  ├→ 反射器 (ReflectionService.evaluate())
  │   五维评分（纯规则，无 LLM 调用）
  │   ├─ score ≥ 0.6 → ✅ 通过 → 进入 LLM 质检
  │   └─ score < 0.6 → ❌ 不通过 → 换 fallback Agent 重试
  │
  ├→ LLM 质量评估 (QualityEvaluationService.evaluate())
  │   四维语义评分（仅反射器 0.50~0.80 边界区间触发）
  │   relevance/completeness/hallucination/helpfulness
  │   ├─ 通过 → ✅ 写入语义缓存 → 返回
  │   └─ 不通过 → ❌ 跳过缓存（防污染），不阻断用户响应
  │
  └→ 错误恢复 (ErrorRecoveryService)
      执行工具时自动解析 error_code JSON
      AgentErrorCode → shouldRetry() → 自动退避重试
```

### 配置项

```yaml
router:
  reflection:
    enabled: true        # 灰度开关
    threshold: 0.60      # 质量阈值（0.0~1.0）
    max-retry: 1         # 最大重试次数
  quality-evaluation:
    enabled: true
    threshold: 0.6
    reflection-lower-bound: 0.50   # 低于此值不走 LLM 质检
    reflection-upper-bound: 0.80   # 高于此值不走 LLM 质检
```

---

## 本地部署混元（Tencent Hunyuan） + canary 灰度接入（2026-07-09）

> 目标：在本地 Ollama 部署混元模型，并以**零风险灰度**方式接入项目统一模型路由层，失败自动回退到现有 qwen/deepseek。

### 1. 本地部署混元（Ollama）

社区已提供混元的 Ollama 移植（`alibayram/hunyuan`，提供 0.5B/1.8B/4B/7B 档；`latest`≈4B，权重 2.6GB）。一键拉取：

```bash
# 默认 latest（4B）；CPU 机型可选更小尺寸（如 :1.8b / :0.5b，具体 tag 以 ollama.com/library/alibayram/hunyuan 为准）
ollama pull alibayram/hunyuan
# 验证模型已就绪
ollama list | grep hunyuan
```

> ⚠️ **网络限制（本开发机已验证）**：`registry.ollama.ai` 的 manifest/小 blob 可达，但 **2.6GB 权重 blob 被 Cloudflare 拦截传输（范围下载 0 字节、连接重置）**，且本机未配置 `HTTP_PROXY/HTTPS_PROXY`（GitHub 直连亦超时）。因此在当前环境**无法完成模型二进制下载**。请在可直连 ollama.com 的机器上执行上述 `ollama pull`，再把模型目录（`OLLAMA_MODELS`，本机为 `D:\ollama_models`）同步过来，或直接在目标机器部署本项目。

### 2. 项目接入（canary 灰度，已完成）

此前 `tier.canary-*` 是**死特性**（配置了比例却未真正把灰度模型接入调用链）。本次补全：

- `TieredModelRouter`：新增 `canaryModel`（ChatModel）字段；`call()` 在灰度命中时把 `canaryModel` 作为**尝试链首节点**，命中即优先用混元，失败沿 `canaryModel → 正常档位 → 降级链` 自动回退，全程可观测（canary 命中/回退打点）。
- `TierModelAutoConfiguration`：`tier.canary-model` 非空时按 `baseUrl` + `canary-model` + `canary-temperature` 构建独立 `OllamaChatModel` 并注入路由器。
- `TieredModelRouterProperties`：新增 `canaryTemperature`（默认 0.3）。
- `isCanaryRequest()` 基于 query hashCode 做**确定性灰度**（同一 query 始终命中/不命中，便于对比评测）。

### 3. 配置项（smart-assistant-router/src/main/resources/application.yml）

```yaml
tier:
  light:
    model: qwen2.5:3b
  standard:
    model: deepseek-r1:7b
  heavy:
    model: deepseek-r1:7b
  degradation-enabled: true
  # ⭐ 混元灰度接入：canary-ratio 由 0.0 翻到 0.1 即开始小流量试接混元
  canary-ratio: 0.0
  canary-model: alibayram/hunyuan   # 预填混元，翻 ratio 即生效
  canary-temperature: 0.3
```

### 4. 验证方式

```bash
# (A) 单元验证 canary 命中/回退逻辑（无需模型，CI 安全）
mvn -pl smart-assistant-common test -Dtest=TieredModelRouterTest

# (B) 真实接入验证（需本机已 ollama pull 成功 + Ollama 在 11434 运行）
#     -Dhunyuan.integration=true 开启集成测试：基础对话 + 工具调用
mvn -pl smart-assistant-common test -Dtest=HunyuanOllamaIntegrationTest -Dhunyuan.integration=true

# (C) 运行期灰度：把 router 的 tier.canary-ratio 设为 0.1，观察日志
#     [TieredRouter] canary 命中，尝试灰度模型=alibayram/hunyuan ...
#     失败则自动回退到 qwen/deepseek，不影响线上
```

### 5. 交付状态

| 项 | 状态 |
|----|------|
| canary 灰度机制补全（代码） | ✅ 完成，19 单测通过 |
| router yml 接入配置 | ✅ 完成（默认关闭，翻 `canary-ratio` 即生效） |
| 集成测试（HunyuanOllamaIntegrationTest） | ✅ 已写，受 `-Dhunyuan.integration` 门控 |
| 混元模型二进制下载 | ⚠️ 本环境网络受限未完成（见 §1 警告），需在可联网机器部署 |

---

## Spring AI 2.0 工程化落地（借鉴 MateCloud 实践）

> 提交 `0e89422`（2026-07-07）：参考 Spring AI 2.0 + MateCloud 生产实践，补齐**内容护栏 / 调用审计 / 统一工厂 / 工具发现 / 类型安全记忆 / 实体化**六大工程化能力，并将其收口到全部 Agent/Consumer/Router 的 ChatClient 调用链。全 11 模块编译通过。

### 改造总览（按优先级顺序）

```
P0-2 调用审计闭环  →  P0-1 内容安全护栏  →  P1-1 统一 ChatClient 工厂
   →  P1-2 @Tool 自动发现  →  P2-1 ChatMemory 抽象  →  P2-2 entity() 封装
   →  Consumer 4 服务接入统一链  →  Router 5 服务接入统一链
```

### P0-1 内容安全护栏（SafeGuardAdvisor）

| 项 | 内容 |
|----|------|
| 类 | `common.rag.advisor.SafeGuardAdvisor` + `common.error.PromptInjectionBlockedException` |
| Order | `50`（前置全部 Advisor，请求进入即拦截） |
| 检测 | 复用 `AgentSafetyService.detectInjection()`（已剔除高误杀规则："你是客服"/"你是一个"） |
| 行为 | 命中抛 `PromptInjectionBlockedException` → 发布 `BLOCKED` 审计事件 → `SmartReActAgent` catch 分支返回友好降级提示 |
| 配置 | `advisor.safe-guard.enabled=true`（默认开） |

### P0-2 调用审计闭环（AiAuditEvent）

| 项 | 内容 |
|----|------|
| 事件 | `common.audit.AiAuditEvent`（record）：traceId / tenantId / provider / model / promptTokens / completionTokens / totalTokens / latencyMs / resultType / approved / userInputDigest / timestamp |
| 发布 | `TokenUsageAdvisor` 重写，真实从 `response.chatResponse().metadata.usage` 抽取 token；`resolveProvider(model)` 推断供应商；`adviseStream` 经 `doOnNext/doOnComplete/doOnError` 发布 |
| 落库 | `AiAuditStore`（内存环形缓冲默认 1000）+ `@EventListener(AiAuditEvent)` 结构化日志 |
| resultType | `{SUCCESS, ERROR, BLOCKED}` |

### P1-1 统一 ChatClient 工厂（AiChatService）

```java
// common.rag.advisor.AiChatService
ChatClient buildChatClient(ChatModel model);              // 自包含：new Builder(model) + applyAdvisors
ChatClient.Builder applyAdvisors(ChatClient.Builder b);   // 在业务自有 Builder 上叠加统一 Advisor 链
```

- `AdvisorChainAutoConfiguration` 注册 `aiChatService` Bean
- 按 null 装配顺序：SafeGuard → TokenUsage → ThinkingCollector → PromptAudit
- **收口范围**：Order/General/Product `AgentConfig` + Consumer 4 服务（`ConversationSummarizationService`/`AnswerPersonalizationService`/`LLMPreferenceExtractor`/`HybridDataQueryService`）+ Router 5 服务（`ModelRoutingService`/`ReflectionService`/`ResultMerger`/`TaskPlannerService`/`SemanticRouteCacheService`→`ReplyFormatter`）
- 消除所有 `defaultAdvisors(...)` 手动样板，统一 Advisor 链（护栏/审计/思考收集/提示审计）一处定义、全局生效

### P1-2 @Tool 自动发现（AiToolRegistry）

```java
// common.tool.AiToolRegistry
ToolCallback[] assemble(Object... toolObjects);   // 聚合所有 @Tool 对象，跳过 null
ToolCallback[] assemble(List<Object> toolObjects);
```

- `AdvisorChainAutoConfiguration` 注册 `aiToolRegistry` Bean
- 四模块工具注册收敛为单行：`assemble(orderTools, analyticsTool, couponTools, ...)`，替代逐条 `MethodToolCallbackProvider` 列举

### P2-1 ChatMemory 抽象 + P2 类型安全 Redis 实现

| 类 | 职责 |
|----|------|
| `common.memory.ChatMemory` | 抽象接口 `add / get / clear` |
| `common.memory.InMemoryChatMemory` | 有界环形缓冲（默认 100 条），单实例默认 |
| `common.memory.MessageCodec` | 显式 `type` 信封 + 自愈 `ObjectMapper`（`JavaTimeModule` + `FAIL_ON_UNKNOWN_PROPERTIES` 关 + `USE_LONG_FOR_INTS` 开），按 `MessageType` 路由回 `UserMessage`/`SystemMessage`/`AssistantMessage` Builder（含 `toolCalls`） |
| `common.memory.RedisChatMemory` | 实现 `ChatMemory`，`StringRedisTemplate` 存 `a2a:chat:memory:{convId}` JSON 数组，有界淘汰 + 可选 TTL |

> ⚠️ **为什么需要 MessageCodec**：`org.springframework.ai.chat.messages.Message` 是接口，运行时为多态子类（`UserMessage`/`AssistantMessage`/`SystemMessage`）。直接用 `RedisTemplate` 序列化 `List<Message>` 会因类型擦除 + 接口无多态判别导致反序列化失败/退化为错误类型 → 记忆数据损坏。显式 `type` 信封规避此问题，且**不动 spring-ai 源码**。

> 💡 通过 profile/配置二选一注册 `RedisChatMemory` Bean，即可切换为分布式多实例记忆实现，与 `InMemoryChatMemory` 行为一致。

### P2-2 实体化结构化输出（entity()）

```java
// common.rag.advisor.AiChatService
<T> T entity(ChatModel model, String userText, Class<T> type);
<T> T entity(ChatModel model, String systemText, String userText, Class<T> type);
```

- 封装 Spring AI 结构化输出（`ChatClient` + `entity()` 绑定 Java 类型）
- 落地示例：`OrderIntentService.record(IntentResult)` 用 `entity(lightModel, sys, user, IntentResult.class)` 替换 `.call().content()` + 脆弱字符串标签匹配；`LLMPreferenceExtractor` 改为不可变 `record ExtractedPreferences` + `entity()` 绑定，删除手工 `ObjectMapper.readTree` 映射

### 验证

- `common` 模块：`MessageCodec`/`RedisChatMemory` 6 测试 + `AiChatServiceEntityTest` 4 测试 全部通过
- `TokenUsageAdvisorAuditTest` / `SafeGuardAdvisorTest` 通过
- 全模块 `bash mvn21.sh -q compile` EXIT=0（11 模块全部通过）
- 备注：Windows 下 `./mvnw` 损坏，须用 `bash mvn21.sh`（JDK21 + Maven 3.9.6 包装）；Spring AI 2.0 流式类型名为 `ChatClient.ChatClientRequestSpec`

---

## 生产级 Agent 架构实战落地（G1/G2，借鉴陈嗣俊《生产级 Agent 架构实战》2026-06-04）

文章指出 Demo→生产级 Agent 须解决**上下文膨胀 / 工具调用风暴 / 缺少可观测性 / 追求 100% 自动化**四类失效。SmartAssistant 已实质落地绝大多数原则（状态机回放、递归滚动摘要、经验体系、审批充血状态机、三层记忆、语义缓存、评测闭环等），本次补齐其中两项明确的工程缺口：**G1 工具同参数去重**与 **G2 步数按入口分级**。

### G2：步数 / 预算按入口分级（ReActProfile）

原 `SmartReActAgent` 各入口统一 `maxIterations=10 / timeoutMs=60s`，无法按入口差异化预算。改为声明式 `ReActProfile` 配置模型：

- **不可变配置模型** `ReActProfile`（record）：集中 `maxIterations / timeoutMs / tokenBudgetRatio / contextWindow / toolTimeoutMs / maxConcurrency`；`DEFAULT = new ReActProfile(10, 60_000, 0.8, 128_000, 30_000, 4)`。
- **注册中心** `ReActProfileRegistry`：`Map<String, ReActProfile>`，提供 `getOrDefault(key, fallback)`。
- **配置绑定** `ReActProfileProperties`：`@ConfigurationProperties(prefix = "smartassistant.react")`，全部字段 Optional。
- **自动装配** `ReActProfileAutoConfiguration`：遍历 properties 构建 Registry，缺失字段以 `DEFAULT` 补齐。
- **接入点**（均 `@Autowired(required=false) ReActProfileRegistry`，缺失自动回退 DEFAULT）：`OrderAgentConfig`(order) / `GeneralAgentConfig`(general) / `ProductAgentConfig`(product) / `McpAgentService`(mcp)，统一调用 `.withProfile(entryKey, reactProfileRegistry)`。

**向后兼容保障**：未配置任何 `profiles.<entry>` 时严格回退 `DEFAULT(10/60s)`，行为与改造前完全一致（零行为变化）。

**配置示例**（运维改 `application.yml` 即生效，无需重编）：

```yaml
smartassistant:
  react:
    profiles:
      general:
        max-iterations: 6        # 简单问答入口步数收紧，防长链风暴
        timeout-ms: 45000
      order:
        max-iterations: 12       # 订单类多步操作允许更多步
        max-concurrency: 6
      product:
        max-iterations: 10
      mcp:
        max-iterations: 10
        timeout-ms: 60000
```

### G1：工具「同参数」去重（防接口 500 后风暴）

原连续无增量检测仅按**工具名**判重复，若某工具因接口 500 反复返回相同错误参数，仍会被判定为"有效进展"而空转。改为**工具名 + 参数指纹**：

```java
// SmartReActAgent 连续无增量检测
String currentHash = toolCalls.stream()
    .map(tc -> tc.name() + "(" + Long.toHexString(argHash64(tc.arguments())) + ")")
    .sorted().collect(Collectors.joining(","));
```

- `argHash64(String)` 采用 **FNV-1a 64-bit** 哈希生成参数指纹，稳定且对参数顺序/空白无关。
- **行为**：同工具**同参数**连续重复 → `NO_INCREMENT_LIMIT(=2)` 触发停止，切断风暴；参数不同（即使同工具名）视为有效进展，正常推进。
- **保守性**：哈希碰撞概率极低（64-bit），且碰撞仅导致**提前停止**（不丢消息），不影响正确性。

### 验证

- 新增 `SmartReActAgentGradingTest`（4 例）：① G1 同参数重复 → 3 轮停止 ② G1 参数每次不同 → 跑满 10 轮 ③ G2 配 `maxIterations=4` → 4 轮停 ④ G2 缺 key → 回退 DEFAULT 10 轮。
- 运行结果：`Tests run: 4, Failures: 0, Errors: 0` — BUILD SUCCESS。
- 全模块 `bash mvn21.sh -q compile` EXIT=0（无回归）。
- 提交：`151adfb`（`ca22434..151adfb main -> main`），10 文件 +468/-37。

### 仍存差距（文章建议项，未做）

| 项 | 说明 |
|----|------|
| G3 模型分级 | ✅ **已实现**：按复杂度选三档 + 平滑降级 + 意图覆盖强制定档。见 [↓ G3 专节](#g3-tier-多模型路由统一模型接入层) |
| G4 观测看板固化 | ✅ **已实现**：新增 `a2a-daily-ops.json` Dashboard，6 面板覆盖请求量/成功率/延迟/Token/Agent健康/路由质量/错误分类。导入 Grafana 即用 |
| G5 KB 切块尺寸 | ✅ **已实现**：`ChunkStrategy.defaultMaxTokens()` 512→1024，`defaultOverlap()` 50→128，对齐 800-1200 中文汉字/chunk 推荐值 |

---

## G3 Tier 多模型路由（统一模型接入层）

对标文章④《腾讯混元架构面经》「多模型路由降级 Tier1/2/3 + 动态路由 + 平滑降级」。SA 原全量固定 `deepseek-r1:7b`，无分级无降级。

### 架构

```
请求 → QueryComplexityClassifier(规则<1ms)
         │ SIMPLE ───→ LIGHT  (qwen2.5:3b, 低成本本地小模型)
         │ MEDIUM  ──→ STANDARD (deepseek-r1:7b, 主力本地模型)
         │ COMPLEX ──→ HEAVY  (deepseek-r1:7b/云端强模型, 高能力)
         │
         ├ 意图覆盖(强制档位): refund_complaint→HEAVY, travel_plan→HEAVY
         │
         └ → 沿降级链调用: HEAVY→STANDARD→LIGHT (LIGHT 永为兜底)
               ├ 成功 → 返回 TierSelection(含模型名/降级标识/时延)
               └ 全失败 → ModelTierUnavailableException → 预设文案
```

### 新增组件（common 中台，跨服务复用）

| 组件 | 类型 | 职责 |
|------|------|------|
| `ModelTier` | enum | 三档 LIGHT/STANDARD/HEAVY，`lower()` 向下降级 |
| `TierModelRegistry` | class | 档位→ChatModel 映射，`fallbackChain()` 生成降级链 |
| `TierSelection` | record | 路由结果快照：请求档位/服务档位/降级标识/时延/响应 |
| `DelegatingOptionsChatModel` | class | 选项覆盖型委托：同一底层模型以不同参数复用 |
| `TieredModelRouter` | class (核心) | `selectTier(query,intent)` 选档 + `call(prompt,query,intent)` 带降级调用 |
| `TieredModelRouterProperties` | properties | 绑定前缀 `tier`：三档 model/temperature + intentOverrides + degradationEnabled |
| `TierModelAutoConfiguration` | @Configuration | `@ConditionalOnBean(OllamaChatModel.class)` 自动装配 |

### 降级链设计

选定档位失败时自动沿 `lower()` 链平滑降级（HEAVY→STANDARD→LIGHT），LIGHT 始终为最终兜底。可通过 `tier.degradation-enabled: false` 关闭降级（strict 模式，失败即抛异常）。

降级时自动记录 Micrometer 指标：

| 指标 | 类型 | 用途 |
|------|------|------|
| `model.tier.selections` | Counter | 档位选择分布 |
| `model.tier.degradations` | Counter | 降级调用次数 |
| `model.tier.latency` | Timer | 端到端路由时延 |

### 集成点

| 集成点 | 变更 |
|--------|------|
| `RouterService.inlineFallback()` | 固定 `lightChatClient` → 按复杂度选档 + 平滑降级（无 TieredModelRouter 时退回原行为） |
| `ModelRouterService` | 死代码修复：依赖不存在的 `defaultChatModel`/`heavyChatModel` bean(启动即崩) → 委托 common `TieredModelRouter`，对外保留 `selectModel/getModelTierName` API |
| `application.yml` | 新增 `tier.*` 配置，含三档模型名/温度 + intentOverrides + degradationEnabled |

### 配置示例（router application.yml）

```yaml
tier:
  light:
    model: qwen2.5:3b
    temperature: 0.1
  standard:
    model: deepseek-r1:7b
    temperature: 0.3
  heavy:
    model: deepseek-r1:7b
    temperature: 0.2
  intent-overrides:
    refund_complaint: HEAVY
    travel_plan: HEAVY
  degradation-enabled: true
```

### 测试

新增 `TieredModelRouterTest`（16 例）：selectTier 复杂度映射 + 意图覆盖 + call 成功路径 + HEAVY→STANDARD 降级 + 全降级链 + strict 模式 + registry null 跳过 + null query 兜底 + Micrometer 计数 + null MeterRegistry 鲁棒性。全部通过。`Tests run: 16, Failures: 0`。

---

## P1/P2 Loop Engineering 确定性约束（对标文章⑥）

对标文章⑥《确定性的 Loop Engineering》「ReAct 只是起点，生产级 Agent 必须要有确定性约束」。SA 原 `SmartReActAgent` 循环已有超时/Token预算/增量检测，新增 6 层确定性约束：

### 改造链路（一轮完整迭代）

```
Pre-AL Gate(纯代码: iter 3/20+阶段+验收清单+硬规则)
    ↓
Agent LLM 执行 → 观察包收集
    ↓
LoopGuardService(确定性快速判断, 80%异常秒判)
    ├ 阻塞标记(blocked/无法继续/需要授权)          → PAUSE_BLOCKED
    ├ 用户决策请求(请选择/是否继续)                  → AWAIT_CONFIRMATION
    └ 基础设施错误(timeout/500/rate limit)          → PAUSE_INFRA
    ↓ (未命中→)
Phase Gate 验收门禁(代码否决LLM)
    ├ file_exists     : 文件在磁盘上的存在性检查
    ├ file_glob_count : 通配符搜索文件数量
    ├ script_check    : 工具调用日志中搜索脚本执行记录
    └ user_confirmation: 结构化用户确认记录
    ↓
连续解析失败保护(3次连续空→暂停) + 无进展计数器(3轮→终止)
    ↓
决策状态机 10 条优先级链路 → 日志记录决策路径 → 继续/暂停/终止/换策略
```

### 新增组件

| 组件 | 模块 | 职责 |
|------|------|------|
| `PreALGate` | common/agent | 执行契约 record：iteration/maxIterations/phase/pendingChecks |
| `LoopGuardService` | common/agent | 纯代码规则检测：3 类 11 组 Pattern，80% 异常秒判 |
| `PhaseGate` | common/agent | 4 种代码级验收类型，代码可否决 LLM |
| `LoopAction` | SmartReActAgent 内部 | 10 路决策动作枚举 |
| `DecisionContext` | SmartReActAgent 内部 | 决策上下文 record |
| `resolveNextAction()` | SmartReActAgent | 10 条优先级链决策方法 |
| `GoalContinuityArbiter` | common/agent | 两级目标连续性裁决：确定性rules→词汇重叠度0.24/0.08→LLM灰色区(置信度<0.72→FUZZY) |
| `FeedbackLog` | common/agent | per-round 结构化反馈记录(mistake_pattern/avoid_next_time/suggested_strategy)，最多20条 |

### 8 层约束对照

| # | 约束 | 实现 | 覆盖场景 |
|---|------|------|---------|
| 1 | Pre-AL Gate | `PreALGate.render()` 注入 SystemMessage | Agent 不��步、不空口宣称完成 |
| 2 | 确定性快速判断 | `LoopGuardService.analyze()` 3 类 Pattern | 阻塞/请求确认/基础设施错误 |
| 3 | Phase Gate 验收门禁 | `PhaseGate.verify()` 4 种检查 | 文件/脚本/确认客观验收 |
| 4 | 连续解析失败保护 | `consecutiveParseFailures ≥3 → pause` | 评估器失效不空转 |
| 5 | 无进展计数器 | `noProgressCount ≥3 → terminate` | Agent 卡住不自知 |
| 6 | 决策状态机 | `resolveNextAction()` 10 路径 | 确定性的退出条件 + 策略切换日志 |
| 7 | 目标连续性裁决 | `GoalContinuityArbiter.arbitrate()` 两级裁决 | 用户中途插嘴→同一任务/新任务/模糊(交用户) |
| 8 | 反馈学习日志 | `FeedbackLog.record()` per-round 结构化记录 | 每轮诊断(mistake_pattern/avoid)经验沉淀 |

### 配置示例

```java
SmartReActAgent agent = new SmartReActAgent(chatModel)
    .withPreALGate(true)
    .withPhaseGate(checks, "数据收集阶段");
```

### 测试

零回归：SmartReActAgentGradingTest(4) + TieredModelRouterTest(16) = 20/20 全部通过。

✅ **已无仍存差距**：文章⑥建议的全部 6 层确定性约束 + 目标连续性裁决 + 反馈学习日志均已实现。

| 项 | 说明 |
|----|------|
| 目标连续性裁决 | ✅ **已实现**：新增 `GoalContinuityArbiter` 两级裁决(确定性rules→词汇重叠度0.24/0.08→LLM灰色区)，集成到 `SmartReActAgent` |
| 反馈学习日志 | ✅ **已实现**：新增 `FeedbackLog` 类，per-round 记录(mistake_pattern/avoid_next_time/suggested_strategy)，最多20条。`SmartReActAgent` 在各返回点自动记录超时/迭代上限/正常完成等反馈 |

---

## G4 观测看板固化

`monitoring/provisioning/dashboards/a2a-daily-ops.json` — 对标文章「每天看 6 指标」固化为 Grafana Dashboard，导入即用。

**9 面板一览：**

| # | 面板 | PromQL |
|---|------|--------|
| ① 请求量 & 成功率 | stat | `sum(a2a_router_routing_requests_total)` + 成功率% |
| ② 延迟 p50/p95/p99 | stat | `histogram_quantile` |
| ③ Token 消耗 | stat | `a2a_llm_token_input/output_total` |
| ④ Agent 健康 | bargauge | `a2a_agent_timeout/max_iteration_hit/context_compress_total` |
| ⑤ 路由质量 | piechart | `a2a_router_multi/single_intent_requests_total` |
| ⑥ 错误分类 | bargauge | `a2a_tool_execution_result{status=}` |
| ⑦ 完成率按 Agent | bargauge | `success_total / requests_total by (service)` |
| ⑧ 档位分布 | piechart | `model_tier_selections_total by (tier)` |
| ⑨ 平均迭代 | stat | `iteration_total / (timeout + max_iter) by (service)` |

### G4 运营 6 指标精简看板（直映射文章指标）

在 `a2a-daily-ops.json`（9 面板）之外，新增 `monitoring/provisioning/dashboards/a2a-ops-dashboard.json` —— **精确映射文章「每天看 6 指标」**，6 面板一眼看完运营健康度。uid=`a2a-ops`，title=`A2A 运营看板 (6 指标)`。

**指标采集层（零 Spring 装配）**：`smart-assistant-common/.../observability/OpsMetrics.java` 基于 `Metrics.globalRegistry` 全局注册表，无需改任何 Bean 装配，对既有 4 模块手写 `new XxxAgent(...)` 测试零破坏。空 tag 经私有 `sanitize()` 兜底为 `"unknown"`。

| 文章指标 | Micrometer 指标 | 类型 | 接入点 |
|---------|---------------|------|--------|
| 平均延迟 | `a2a_route_latency_seconds{agent,intent}` | Timer | `RouterService.route()` finally 块 |
| 工具失败率 | `a2a_tool_calls_total{agent,outcome}` | Counter | `SmartReActAgent.executeToolCallWithRetry` 6 个终态 |
| 单轮 Token | `a2a_turn_tokens{agent}` | DistributionSummary | `SmartReActAgent` LLM usage 提取（≤0 跳过） |
| 无答案率 | `a2a_answers_no_evidence_total{agent,intent}` | Counter | Order/Product 无证据拒答短路分支 |
| 人工接管率 | `a2a_handoffs_total{reason,agent}` | Counter | `RouterService` HEAVY 情绪兜底 |
| 缓存命中率 | `a2a_semantic_cache_hits_total{cache}` / `a2a_semantic_cache_misses_total{cache}` | Counter | `SemanticRouteCacheService` 命中/未命中 |

**6 面板 PromQL（timeseries）：**

| 面板 | PromQL |
|------|--------|
| 平均延迟(ms) | `rate(a2a_route_latency_seconds_sum[5m]) / rate(a2a_route_latency_seconds_count[5m]) * 1000` |
| 工具失败率(%) | `sum(rate(a2a_tool_calls_total{outcome="failure"}[5m])) / sum(rate(a2a_tool_calls_total[5m])) * 100` |
| 单轮 Token | `sum(rate(a2a_turn_tokens_sum[5m])) / sum(rate(a2a_turn_tokens_count[5m]))` |
| 无答案率(%) | `sum(rate(a2a_answers_no_evidence_total[5m])) / sum(rate(a2a_answers_total[5m])) * 100` |
| 人工接管率(%) | `sum(rate(a2a_handoffs_total[5m])) / sum(rate(a2a_answers_total[5m])) * 100` |
| 缓存命中率(%) | `sum(rate(a2a_semantic_cache_hits_total[5m])) / (sum(rate(a2a_semantic_cache_hits_total[5m])) + sum(rate(a2a_semantic_cache_misses_total[5m]))) * 100` |

**测试覆盖**：`OpsMetricsTest`（7 用例，基于 `SimpleMeterRegistry` + `Metrics.addRegistry/removeRegistry` 隔离）：
- outcome 分流（success / failure）
- 延迟 Timer 记录
- Token 非负（≤0 跳过）
- 应答 / 无证据计数
- 人工接管计数
- 缓存命中 / 未命中计数
- 空 tag 兜底（`sanitize` → `"unknown"`）

部署：Grafana provisioning `dashboard.yml` 已指向 `provisioning/dashboards/` 目录，JSON 放入即自动加载，无需重启 Grafana 进程。

---

## P0 熔断半开 + 多级优先级队列

### 熔断半开（DegradationService）

熔断器三态：`NORMAL → HALF_OPEN → LIGHT/HEAVY`

```
错误率 20%~40% → LIGHT（跳过复杂 DAG）
错误率 >40%    → HEAVY（回退内联回复）
错误率降至阈值 → HALF_OPEN（非直接 NORMAL）
   ↓ 30s 冷却后
RouterService 放行 inlineFallback 探测
   ↓
recordProbeResult(true)  → NORMAL（恢复）
recordProbeResult(false) → HEAVY（继续降级）
```

### 多级优先级队列（RequestQueueService）

| 级别 | 常量 | 适用场景 |
|------|------|---------|
| REALTIME | 10 | 通用对话、简单查询（低延迟） |
| NORMAL | 5 | 订单/商品查询等业务 |
| BATCH | 0 | 数据分析、批量处理（闲时消费） |

通过 `priority` 参数（GET/POST）传入，`PriorityBlockingQueue` 按值排序，同优先级 FIFO。配置 `queue.priority-enabled=true`。

---

## P1 小模型前后置 + 灰度发布

### 小模型前后置（LightweightPrePostProcessor）

小→大→小三段流水线：

```
用户请求 → 小模型前置(意图验证+查询规范化) → 大模型(核心推理+工具调用) → 小模型后置(质量校验 0-1 评分) → 返回用户
```

- `preProcess(query, intent)`: 小模型验证意图匹配，返回 `PreProcessResult(match/refinedIntent/suggestion)`
- `postProcess(question, reply)`: 小模型校验回复质量，返回 `PostProcessResult(pass/score/suggestion)`
- 异常不影响主流程，仅日志记录

### 灰度发布调度（TieredModelRouter）

按比例渐近切流，`canary-ratio: 0.1` 即 10% 流量走新模型：

```yaml
tier:
  canary-ratio: 0.1          # 10% 流量走灰度
  canary-model: qwen2.5:32b  # 新模型名
  degradation-enabled: true
```

`isCanaryRequest()` 基于 query hashCode 确定性灰度（同一 query 始终落在同一区间，保证用户一致性）。

---

## P2 执行 DAG 可视化 + 分组完成率看板

### 执行 DAG 可视化

基于 Redis `routing:sse:events:{requestId}` SSE 事件流构建节点状态链：

```
GET /api/admin/execution-dag/{requestId}     → 结构化 DAG 数据（JSON）
GET /api/admin/execution-dag/{requestId}/view → 内联 HTML 可视化页面
```

节点状态着色：🟢 completed / 🔵 running（脉冲动画） / 🔴 failed / 🟡 skipped / 🟣 replan。链式边按执行顺序自动生成。

---

## RAG 索引版本化 + 对账服务

对标文章⑦《RAG 索引重建与向量库同步》。

### 索引版本号

`KnowledgeDocument` 新增 `indexVersion` 字段，`ingestion pipeline` 注入 `CURRENT_INDEX_VERSION="v1"`。索引策略/embedding模型变化时递增此常量，检索时按 `indexVersion` 过滤。

### 索引对账服务

`IndexReconciliationService` 提供：

| 方法 | 说明 |
|------|------|
| `reportIndexingResult(store, baseId, chunkCount, vectorCount, success)` | 上报索引操作结果 |
| `checkConsistency()` | 汇总对账报告（stores/totalChunks/totalVectors/missingCount/consistent） |
| `cleanOldRecords(maxRecords)` | 清理过期记录 |

线程安全（ConcurrentHashMap + AtomicInteger），供 KnowledgeBase 实现或定时任务调用。

## @Tool 统一日志切面

系统通过 AOP 切面自动拦截全部 38 个 `@Tool` 方法，统一记录调用日志，无需在每个工具方法内手动写日志。

### 架构设计

```
Consumer (requestId 生成)
  │  ToolLogContext.setRequestId(requestId)
  │
  ├→ Router (HTTP 直调)
  │    │  AgentCallerService.enrichWithRequestId(instruction, requestId)
  │    │  → instruction 前缀注入: "[requestId:xxx]\n原始指令"
  │    │
  │    └→ Agent 服务
  │         │  ToolLogAspect (@Around)
  │         │  → 从 instruction 参数提取 requestId
  │         │  → MDC 设入，工具调用日志关联同一 requestId
  │         │
  │         └→ @Tool 方法执行
  │              │  ToolLogAspect (@Around)
  │              │  → MDC.get("toolRequestId") 获取 requestId
  │              │  → log.info("[Tool] method={} requestId={} params={} result={} duration={}ms")
  │              │
  │              └─ finally: ToolLogContext.clear()
```

### 日志格式

```
[Tool] method=calculate requestId=req-abc123 params={expression: "2+3*4"} result=14 duration=3ms
[Tool] method=getHotNews requestId=req-abc123 params={} result=[5条新闻] duration=222ms
```

- 参数截断至 300 字符，结果截断至 500 字符
- 异常时输出 `log.warn` 级别，包含 error 信息

### 新增监控指标（2026-07-02）

| 指标名 | 类型 | 标签 | 说明 |
|--------|------|------|------|
| `a2a_tool_error_type` | Counter | tool,error(retryable/fatal) | 按错误类型分类工具错误 |
| `a2a_tool_execution_latency` | Timer (p50/p95/p99) | tool | 含延迟分位数直方图 |
| `a2a_tool_execution_result` | Counter | status(retryable_failed/fatal_failed) | 错误分类计数器 |
| `a2a_tool_execution_latency_percentile` | Timer (p50/p95/p99) | — | 按 ErrorType 的延迟分位数 |

### 核心组件

| 组件 | 位置 | 说明 |
|------|------|------|
| `ToolLogContext` | `common/tool/` | ThreadLocal + MDC 持有 requestId |
| `ToolLogAspect` | `common/tool/` | `@Around` 拦截 @Tool，记录输入/输出/耗时 |
| `spring-boot-starter-aop` | `common/pom.xml` | AOP 依赖（optional） |

---

## 语义缓存

系统在 Router 模块实现了三层语义缓存，全部本地执行无需外部 API 调用。Tier 3 使用 **BGE-small-zh ONNX(512d)** 本地模型，零网络依赖。

### 缓存架构

```
getCachedDecision(question)
├── Tier 1: 精确匹配 MD5(question)         → ~1ms    Redis GET
│   └── 命中后如果 reply 不存在，兜底 keyword reply
│
├── Tier 2: 关键词哈希(分词→排序→MD5)     → ~5ms    Redis GET（无需 LLM）
│   └── "上海天气怎么样" 和 "上海天气如何" → 相同 hash
│   └── 回复缓存首次执行后立即保存
│
└── Tier 3: BGE ONNX 向量匹配(余弦≥0.70)   → ~5ms    本地 ONNX Runtime
    └── BGE-small-zh-v1.5 ONNX (512d)，BERT 标准 tokenization
    └── 自动降级 TF 向量（模型文件缺失时）
    └── VectorCacheStore 内存索引，最大 10k 条
    └── 阈值 0.70 覆盖前缀扩展（"北京天气"→"北京天气冷不冷"）
```

### 回复缓存策略

系统在 `saveReply()` 时按以下顺序判断是否跳过回复缓存（路由决策始终缓存）：

```
saveReply() 时
├── 回复缓存总开关 reply-cache-enabled=false?
│   └── ✅ 是 → 跳过（P3-B 文章⑥：彻底不缓存可变的最终回答，仅保留路由决策加速）
│
├── 会话复述类提问?（"我刚刚问了什么"、"再说一遍"）
│   └── ✅ 是 → 跳过（回答仅对当前会话有效）
│
├── 管理员工具执行标记存在? (Redis admin:tool:called:latest)
│   └── ✅ 是 → 跳过（Agent 端 @AdminOnly 工具触发后写入）
│
├── Agent 声明 cache-ttl-seconds < 3600 ?
│   └── ✅ 是 → 跳过（天气类 ttl=1200，每次拿最新数据）
│
├── 意图被问到 ≥2 次?（即高频意图）
│   └── ❌ 否 → 跳过（低频问题缓存也不会命中）
│
└── 通过 → 缓存回复（关键词级 + 意图级，key 含租户+版本作用域）
```

#### 缓存 key 作用域（P3-B，文章⑥：权限 + 索引版本进入缓存 key）

回复类缓存 key 一律注入 `{tenant:version}` 作用域，避免把"受权限/版本影响可变的最终回答"跨请求复用：

- **租户隔离**：`AclContext.fromMdc().getTenantId()` 进入 key，租户 A 的缓存回复不会被租户 B 命中（防跨租户数据泄露）。
- **版本失效**：`CacheVersionManager.getCurrentVersion()` 进入 key，索引/知识库版本递增后旧版本回复 key 自然失效。

| 缓存类型 | Key | 写入时机 | TTL |
|---------|-----|---------|:---:|
| 路由决策 | `a2a:route:semantic:{md5(intentTag)}` | 每次路由后 | 24h |
| 精确映射 | `a2a:route:exact:{md5(question)}` | 每次路由后 | 24h |
| 关键词路由 | `a2a:route:keyword:{md5(keywords)}` | 每次路由后 | 24h |
| **关键词回复** | `a2a:route:keyword:reply:{tenant:version}:{md5(keywords)}` | **Agent 执行后（TTL≥1h 时）** | 动态 |
| 意图回复 | `a2a:route:reply:{tenant:version}:{md5(intentTag)}` | 被问到 ≥2 次后（TTL≥1h 时） | 动态 |

> 路由决策 key（`semantic`/`exact`/`keyword`）仍按原租户无关设计保留——路由决策是"去往哪个 Agent"的判定，与用户数据无关，可安全跨租户共享以加速；而**回复内容**因携带用户私有数据，必须按租户+版本隔离。
>
> 运维可通过 `router.semantic-cache.reply-cache-enabled=false` 完全关闭最终回复缓存（默认 `true`，保留高频 FAQ 收益），彻底对齐文章⑥"反对缓存最终回答"的建议。

### 缓存版本一致性

Router 和 Consumer 之间的缓存通过 `CacheVersionManager` 保持一致性：

```
Router (经验变更时)
  ExperienceService.saveExperience()
    └→ cacheVersionManager.incrementVersion()
         └→ Redis: a2a:cache:version = 3

Consumer (读取缓存前)
  VectorSearchCacheService.searchWithCache()
    └→ 缓存值格式: "3|answer text"
    └→ isCacheVersionValid("3|...", 当前版本)
         ├→ 版本匹配 → 返回缓存
         └→ 版本不匹配 → 跳过缓存，重新生成
```

- 旧版缓存（无 `{version}|` 前缀）默认通过，兼容过渡期
- 本地 5 秒版本缓存，避免每次查询都读 Redis

### 动态 TTL（按 Agent 类型 + 问题内容）

| Agent | 场景 | TTL | 说明 |
|:-----|------|:---:|------|
| location_weather | 纯天气查询 | **20min** → 低于 1h 阈值，回复不缓存 | 气温/降水实时更新 |
| location_weather | 景点/推荐/去哪 | **12h** | 景区信息几乎不变 |
| food_recommendation | 今日推荐 | **2h** | 每日特价更新 |
| food_recommendation | 一般美食查询 | **12h** | 餐厅信息稳定 |
| general_chat / builtin | 闲聊/问答 | **2h** | 事实性回答稳定 |

### 回复前缀个性化

根据提问者是否为同一用户，自动选择不同前缀：

| hitCount | 同用户 | 不同用户 |
|:--------:|--------|----------|
| == 2 | `再帮你查一次，结果和之前一样～` | `查询结果如下：` |
| ≥ 3 | `（以下是我之前查到的信息）` | `（以下是根据历史查询获取的信息）` |
| > 6h | `📅 根据6小时前查询的信息` | `📅 以下信息来源于6小时前的数据` |

### 性能效果

| 场景 | 典型耗时 | Agent 调用 |
|:----|:-------:|:---------:|
| 冷启动（无缓存） | 5-18s | ✅ 执行 |
| 路由缓存命中（Tier 1/2 无回复） | 3-15s | ✅ 执行 |
| **全缓存命中（Tier 1/2/3）** | **1-5ms** | **❌ 跳过** |
| BGE 向量匹配命中（前缀扩展兜底） | **5ms** | ❌ 跳过 |

每意图仅需 **1-2 次 Agent 执行**（改前需 5 次），缓存命中时延迟从 5-18s 降至 **1-5ms**。

---

## P4 情绪护栏与记忆版本治理

P4 对应两篇参考文章的落地：P4-A 情绪干预（文章②安全护栏）在 Router 入口增加**确定性情绪风险分级**；P4-B 记忆版本留存（文章①记忆治理）在 Consumer 端实现**冲突记忆的版本化留存而非物理覆盖**。两项均为「分级兜底 + 非破坏式更新」思路，避免对正常链路引入 LLM 开销。

### P4-A 情绪干预（Router，对标文章②）

路由入口 `route()` 在调用 `guardrailService.check()` 之后立即执行 `guardrailService.checkEmotion(question)`，做**关键词确定性检测**（不含 LLM，零额外延迟）：

- 关键词分 4 类信号：`SELF_HARM`（自伤）、`DEPRESSION`（抑郁）、`ANGER`（愤怒）、`ANXIETY`（焦虑）。
- 命中后取最高等级 `NONE / LIGHT / MEDIUM / HEAVY`：
  - **HEAVY**（自伤/伤人倾向）→ `route()` **安全早返回**：直接返回求助引导文案，`disableTools=true` + `requiresHumanHandoff=true`，**绝不调用任何 Agent 工具**，引导用户联系全国 24 小时心理危机干预热线 `400-161-9995`。
  - **LIGHT / MEDIUM** → 通过 `applyEmotion()` 在 `finalizeRouting` / 兜底路径统一注入结果对象，下游 Agent/Consumer 可据此调整回复语气或限制高危工具。
- `RouteDecision` / `RoutingResult` 新增 4 字段透传：`emotionLevel` / `emotionIntervention` / `disableTools` / `emotionGuidance`（均用 `@Builder(toBuilder=true)` 便于后处理追加）。

| 文件 | 职责 |
|:-----|:-----|
| `guardrail/EmotionLevel.java` | 情绪等级枚举（顶级类） |
| `guardrail/EmotionCategory.java` | 情绪类别枚举（自伤/抑郁/愤怒/焦虑） |
| `guardrail/EmotionCheckResult.java` | 检测结果 record（`triggered()` / `none()`） |
| `service/core/RouterService.java` | 入口检测 + HEAVY 早返回 + `applyEmotion()` 后处理 |

> 检测逻辑为**确定性关键词**，避免给每次请求叠加 LLM 判断；HEAVY 路径是最高优先级兜底，优先于所有路由策略与缓存命中。

### P4-B 记忆版本留存（Consumer，对标文章①）

UserProfileService 抽取偏好时不再「新值直接覆盖旧值」，而是写入独立的版本化记忆库 `MemoryVersionStore`（文件存储 `data/users/{userId}/memories.json`，Jackson + JavaTimeModule）：

- **来源优先级** `MemorySource`：`EXPLICIT(3) > FACT(2) > INFERRED(1)`。`recordMemory()` 按类别注入来源（FOOD_PREF/TRAVEL_PREF/LOCATION 用 INFERRED；BUDGET/DIETARY 用 EXPLICIT）。
- **冲突时不物理删除旧记忆**：
  - 无 ACTIVE 同键 → 新建 v1；
  - 同值 → 仅更新来源/时间；
  - 取值冲突 → 来源 rank 高者胜：胜则旧记忆标记 `SUPERSEDED`（记录 `supersededAt` / `supersededBy`，同 `lineageId` 血缘）、新版 `version+1`；败则保留旧值、丢弃新值。
- `MemoryEntry` 为 `@With` record，支持不可变更新；提供 `getActive` / `getAllVersions(lineageId)` / `getHistory` 追溯接口。
- 写入通过 `UserProfileService.recordMemory(...)` 调用，**异常安全**（catch 仅记 debug 日志，失败不影响主流程）。

| 文件 | 职责 |
|:-----|:-----|
| `service/memory/MemorySource.java` | 来源枚举与 `rank()` 优先级 |
| `service/memory/MemoryStatus.java` | `ACTIVE` / `SUPERSEDED` |
| `service/memory/MemoryEntry.java` | 记忆条目 `@With` record（含 `lineageId` / `version` / `status`） |
| `service/memory/MemoryVersionStore.java` | 版本化读写与冲突裁决 |
| `service/recommendation/UserProfileService.java` | 偏好抽取后调用 `recordMemory()` |

### P4 测试覆盖

| 测试类 | 用例数 | 覆盖点 |
|:------|:-----:|:------|
| `GuardrailServiceEmotionTest` | 8 | 无情绪→NONE、焦虑→LIGHT、愤怒→MEDIUM、抑郁→MEDIUM、自伤→HEAVY（禁用工具+人工介入+热线）、自伤优先、关闭、空白 |
| `RouterServiceEndToEndTest#emotionHeavyReturnsSafeResult` | 1 | HEAVY 触发安全早返回，断言 `emotionLevel/intervention/disableTools` 及 `verify(agentCallerService, never()).callAgent(...)` |
| `MemoryVersionStoreTest` | 5 | 新增 v1、同值无新版本、冲突高优先级胜（旧 SUPERSEDED 留存）、冲突低优先级败（历史仅 1 条）、时间优先同来源递增 |

---

## P5 生产 Faithfulness 校验（文章 Q⑩ 校验层）

P5-A 补齐面试题 Q⑩「减少幻觉（召回 / 生成 / 校验三层）」中的**校验层**：离线评测早已建成 `HallucinationDetector`（4 层规则检测），但此前只被 `RAGEvaluator` / `EvaluationReportService` 引用，**从未接入生产 RAG 回答链路**。本改动将其部署到 Order / Product 的最终回答产出后。

### 实现

- 新增可复用组件 `common.rag.eval.FaithfulnessGuard`（纯 POJO，无 Spring 依赖，可直接单测）：
  - `check(answer, context)` → `FaithfulnessVerdict{checked, hallucination, score, claims, message}`；
  - 关键防御：**上下文为空 / 回答为空 → 跳过（`checked=false`）**，避免 `HallucinationDetector` 对"无上下文"返回 `hasHallucination=true` 的误判；
  - `hallucinationRate >= 0.6`（默认阈值，可构造调节）才视为命中；
  - `guard(answer, context)` 命中时返回「回答 + 免责声明」，未命中返回原回答。
- **Order**：`OrderAgentController` 在 `orderAgent.execute()` 后、以 `qr.getContent()`（RAG 上下文）做后置校验，命中则追加免责声明并 `log.warn` 埋点（score + claims 数）。
- **Product**：`StreamingProductAgentService` 在 `productAgent.execute()` 后、以注入的 `ragContext` 做同等校验（上下文提升到外层作用域以跨过 RAG 检索块）。
- 设计原则：**非阻断**——命中高幻觉时不拒绝回答，仅追加「以上回答部分内容未能在检索资料中核实，仅供参考」免责声明，保证体验与可观测性。

| 文件 | 职责 |
|:-----|:-----|
| `common/.../rag/eval/FaithfulnessGuard.java` | 生产链路忠实度护栏（复用 HallucinationDetector） |
| `order/.../controller/OrderAgentController.java` | 回答产出后校验（上下文取自 `RetrievalQualityResult`） |
| `product/.../service/agent/StreamingProductAgentService.java` | 回答产出后校验（上下文取自 RAG 注入） |

### 修复的预存阻塞

Product 模块此前因以下预存错误从未成功编译，阻碍了 P5-A 在 Product 侧落地，一并修复：

- `product/pom.xml` 缺失 `spring-boot-starter-data-redis`（`ProductAgentConfig` 引用 `StringRedisTemplate`）→ 补依赖（与 order 一致，`StageTraceRecorder` 缺省降级内存）；
- `ProductGuideService.java:77` `result.highQuality()` → `result.isHighQuality()`（同名静态工厂易混）；
- `ProductGuideService.java:77` `result.content()` → `result.getContent()`。
- `OrderAgentController` / `StreamingProductAgentService` 的 `String result;` 存在明确赋值隐患（catch 中 `throw` 导致 finally 引用未初始化），改为 `String result = null;` 消除编译告警。

### P5-B 用户纠正信号闭环（BadCase ③ + CorrectionService 反哺）

**对标文章⑨「经验质量」**：Bad Case 挖掘策略③「用户纠正信号：同一 session 内推翻前次意图」此前标注"待实现"。本项落地该信号，并接入已有的 `CorrectionService`（各 Agent 纠错记录 MD 文件，GeneralTools 已接 `queryCorrections`），形成"检测→持久化→后续不再犯"的闭环。

**设计要点**
- `CorrectionSignalDetector`（确定性关键词检测，零 LLM 开销）：命中「纠正类」中文表达（"你错了/说错了/应该是/纠正一下/我指的是/不是这个/记错了/说反了…"）即判定为纠正信号，并对纯赞同（"你说得对"）做排除，宁少勿滥。
- `BadCaseMinerService.recordCorrection(RoutingDecision)`：命中纠正信号时①写 Bad Case（reason 标注 `用户纠正信号 (③)` + 命中关键词）②若 `CorrectionService` 可用，把纠正持久化到对应 Agent 的纠错记录（后续回答前经 `queryCorrections` 查询，避免重复犯错）。
- `RouterService.route()` 在现有 `record()` 后复用同一 `RoutingDecision` 追加 `recordCorrection()`，**非阻断**（全程 try/catch，异常仅记日志，绝不影响主路由）。

| 文件 | 职责 |
|:------|:------|
| `router/.../service/evaluation/CorrectionSignalDetector.java` | 确定性纠正信号检测（纯 POJO，可单测） |
| `router/.../service/evaluation/BadCaseMinerService.java` | 新增 `recordCorrection()` 落地策略③ + 可选注入 `CorrectionService` |
| `router/.../service/core/RouterService.java` | `route()` 复用 `RoutingDecision` 追加 `recordCorrection()` |
| `common/.../correction/CorrectionService.java` | 复用既有 `appendCorrection()` 持久化纠正（已存在，未改动） |

### P5 测试覆盖

| 测试类 | 用例数 | 覆盖点 |
|:------|:-----:|:------|
| `FaithfulnessGuardTest` | 6 | 回答被支撑→不触发、数字不匹配→命中+追加声明、实体不匹配→命中、无上下文→跳过、空回答→跳过、自定义高阈值抑制 |
| `OrderAgentControllerTest` | 2 | 无证据拒答短路、正常路径注入上下文+调用 Agent（修复 buildEnhancedMessage 未 stub 导致的 NPE） |
| `StreamingProductAgentServiceTest` | 4 | 拒答/高质量注入/兜底/旧链路（回归） |
| `CorrectionSignalDetectorTest` | 6 | 显式纠正/推翻前答/正常Query忽略/纯赞同忽略/空串/null/返回命中词供审计 |
| `BadCaseMinerServiceCorrectionTest` | 4 | 纠正信号→写BadCase+持久化CorrectionService、非纠正不写、纯赞同不写、无CorrectionService时仅写BadCase |

### P5-C 检索 Top-K 自适应治理（文章 Q⑦）

**问题**：`RrfFusionHandler` 在 rerank **之前**就用硬编码 `TOP_K=5` 截断候选，导致 rerank 只能看到 ≤5 条、无法真正精排提纯；且最终注入上下文条数对所有查询一视同仁，开放式/比较型查询信息覆盖不足、事实型查询又易被噪声挤占。

**交付**：
- `AdaptiveRerankTopK`（common）：零 LLM 确定性映射——开放式/比较型/攻略型（"对比/推荐/排行/怎么选/区别"）→ `maxK`；事实型/查数型（"多少钱/电话/营业时间"）→ `minK`；其余 → `defaultK`。边界 `[minK,maxK]` 自动归一化。
- `RerankHandler` 新增可选 `topKResolver`：`rerank` 后按逐查询解析的 K 做最终截断（固定 K 构造器向后兼容，`null` resolver 退化为原行为）。
- `RrfFusionHandler` 硬编码 `TOP_K=5` → 可配置 `candidatePoolK`（默认 20），让 rerank 前候选更丰富。
- `ProductRagPipelineConfig`：`rerank.top-k`(默认5) 作默认/上限、`top-k-min`(3)/`top-k-max`(8) 限定边界，注入 `AdaptiveRerankTopK` 到 rerank handler。

| 新增/改动文件 | 说明 |
|:------|:------|
| `common/.../pipeline/AdaptiveRerankTopK.java` | 新增：自适应 K 策略（minK/defaultK/maxK + 关键词映射） |
| `common/.../pipeline/RerankHandler.java` | 新增 `topKResolver` 参数与自适应截断分支 |
| `product/.../search/handler/RrfFusionHandler.java` | `TOP_K`→`@Value candidatePoolK`(默认20) |
| `product/.../config/ProductRagPipelineConfig.java` | 注入 `AdaptiveRerankTopK` bean + 三个 K 配置项 |

### P5-D 技术债清理

**① `KeywordFastRouteService.parseJsonRules` Jackson 实现**：原 `TODO` 死代码永远返回空列表——外部 JSON 规则配置（如 `keyword-routes.json`）根本不生效。已用 Jackson `ObjectMapper` 反序列化为 `List<KeywordRule>`，支持裸数组与 `{rules:[...]}` 两种形态，解析失败降级为空（内置默认规则兜底），方法降为包可见以便单测。

**② 评测门禁固化（Trial×pass^k 成为默认 Gate）**：`GoldenSuiteEvalGate` 新增 `run(suiteResource, reportDir, agentExecutor)` 重载——注入真实 `TrialRunner.TrialExecutor` 时自动启用硬化默认配置（`enableAgentGate=true`、`agentTrialCount=5`、`minAgentPassKRate=0.8`、`minAgentPassRate=0.8` 等），把"多 Trial 稳定性门禁"设为标准放行条件；未注入运行器时退化为离线占位，向后兼容。配套测试 `GoldenSuiteEvalGateDefaultTest` 验证硬化默认配置生效。

| 新增/改动文件 | 说明 |
|:------|:------|
| `router/.../routing/KeywordFastRouteService.java` | `parseJsonRules` Jackson 实现（去 TODO）+ `JSON_MAPPER` 字段 + 依赖导入 |
| `common/.../eval/GoldenSuiteEvalGate.java` | 新增默认硬化 Gate 重载 `run(suite, reportDir, executor)` |
| `router/.../routing/KeywordFastRouteServiceParseJsonTest.java` | 新增：裸数组/对象包裹/字段映射/降级空/容错 |
| `common/.../eval/GoldenSuiteEvalGateDefaultTest.java` | 新增：硬化默认配置生效 |

### P5 测试覆盖（补充）

| 测试类 | 用例数 | 覆盖点 |
|:------|:-----:|:------|
| `AdaptiveRerankTopKTest` | 6 | 开放式→maxK、事实型→minK、默认、空串/null→defaultK、边界归一化、asResolver 注入 |
| `RerankHandlerTest` | 6 | 固定 K 截断（回归）+ 自适应 resolver 覆盖 |
| `KeywordFastRouteServiceParseJsonTest` | 5 | 裸数组/对象包裹/字段映射/降级空/容错 |
| `GoldenSuiteEvalGateDefaultTest` | 2 | 硬化默认配置生效（enableAgentGate/agentTrialCount/minAgentPassKRate） |

---

## 服务说明

| 服务 | 端口 | 职责 |
|------|------|------|
| **Gateway** | 8081 | API 统一入口，JWT 认证，Redis 限流，负载均衡 |
| **Consumer** | 8082 | SSE 流式对话入口，**三层请求排队流控**(L1全局/L2会话级/L3优先级)，上下文管理，用户画像**时间衰减**，对话记忆沉淀，SSE **断线续传**(Sorted Set O(log n) + EventSource Last-Event-ID)，**Agent 事件总线异步消费**，**请求取消端**(`POST /chat/cancel` 释放槽位)，提供 `/api/data/query` 数据查询独立端点 |
| **Router** | 8083 | 多 Agent 协作路由，**三层语义缓存**，任务分析(意图/置信度/实体/约束/风险/工具评分)，**评测后处理**(实体归一/词槽/澄清/置信度阈值)，**意图查询改写**，**意图向量化检索**(BGE Top-K 动态注入)，DAG 图分解→拓扑并行执行(**指数退避重试**+**验收标准检查**+**重规划触发**+**节点级熔断**)→**Handoff 显式交接**(同步HTTP/异步EventBus可选，含**独立节点级熔断**)，**异常率阈值降级**(DegradationService: >20%轻降>40%重度)，**Agent HTTP 调用熔断**(Resilience4j CircuitBreaker 50%→30s半开)，**二阶段质量评估**(反射器+LLM质检)，**标准错误码表驱动恢复**，**Agent 级工具自动重试**，**经验验证+淘汰**(定时清理低质量经验)+**BGE 语义合并**(cosine≥0.85自动合并)，**摘要回指**(六类实体提取+结构化历史引用)，Nacos 服务发现 |
| **Embedding Service** | 8091 | 独立 BGE ONNX 嵌入服务（1024维），供 Router 语义缓存、知识库检索和 ExperienceService 经验匹配 |
| **Order** | 8085 | 订单查询(Text-to-SQL)，退款处理，物流跟踪，优惠券查询，**BGE 知识库检索 + bge-reranker 重排序**，**独立记忆**(Markdown 文件存储，savePreference/recallMemories) |
| **Product** | 8084 | 商品查询，库存检查，价格查询，**BGE 知识库检索 + Pipeline RAG**(MultiQuery→QueryRewrite→ExactMatch→Keyword→BM25→Knowledge→Graph→RRF→Dedup→Rerank)，**独立记忆**(商品偏好/价格区间/品牌偏好)，**P3 商品图谱**(15节点/36边，5种关系类型)，**Skill 技能包**(比价/库存预警/知识问答) |
| **User** | 8086 | 用户注册登录，JWT Token 签发，角色管理 |
| **General** | 8087 | 闲聊问答，新闻热点，单位转换(温度/长度/重量/货币)，**沙箱保护的多步脚本执行**(ScriptSandbox 三层防御: 关键字黑名单/资源限制/超时熔断)，**独立记忆**(回复风格/常用单位/兴趣话题)，支持风格切换 |
| **Recommend** | **8088** | **🎯 P3 跨模块推荐服务**。三策略推荐引擎：图谱推荐(ProductGraphService)→协同过滤(OrderGraphService 用户购买历史)→热门兜底。Feign 调用 Product+Order REST API，**含 FallbackFactory 熔断降级**(ProductFeignClientFallbackFactory/OrderFeignClientFallbackFactory 返回空列表避免上游异常) |
| **Tool-Registry** | **8090** | **🎯 MCP 兼容工具注册中心 + 发现聚合网关**。中心目录管理所有 `@Tool` 与 MCP-backed 工具（SQL 等）的注册/状态/标签/风险/能力元数据；以 MCP Server（WebMvc/SSE）暴露 `tools/list` 全量发现 + `search_tools` 能力过滤。作为 MCP **client** 拉取后端 MCP Server 工具列表，同步进中心目录（`McpToolSourceIngestor`，含 `inputSchema`/`endpoint`/命名 `namespace.toolName` 防冲突）。自身**不实现**传递式 `tools/call`（执行由 Agent 侧 `McpBackendToolExecutor` 经 `ToolGateway` 治理链转发） |

### ⭐ Router 新增模块 (2026-07-07)

| 模块 | 说明 |
|------|------|
| `service.checkpoint.GraphCheckpointService` | Checkpoint 持久化(Redis)，支持断点恢复。对标 LangGraph Checkpoint |
| `model.GraphState` | 类型化共享 State，3种 Reducer 模式(OVERWRITE/APPEND/MERGE_MAP)。对标 LangGraph `MessagesState` |
| `model.IntentGraph` 增强 | 条件边(ConditionType 枚举)+重路由(rerouteTarget 循环)+HITL标记。对标 LangGraph 全 Graph 特性 |

---

## 环境要求

### 开发环境

- **操作系统**：Windows 10/11 或 macOS / Linux
- **Java**：JDK 21+（推荐 Eclipse Temurin 21.0.6+，`D:\Program Files\Java\jdk-21.0.6+7`）
- **Maven**：3.9+（可选，使用 `bash mvn21.sh` 自动调用系统 Maven）
- **Node.js**：18+（前端构建）
- **Docker**：24+（基础设施服务）
- **Git**：2.x（版本管理）

### 一键安装脚本

```powershell
# 检查依赖
bash mvn21.sh -v        # ⭐ 推荐：绕过 Git Bash 的 Maven 路径兼容问题
.\mvnw.cmd --version    # 备用：Windows CMD 下使用（需先设置 JAVA_HOME）
java -version
node -v
docker --version
```

> ⚠️ **重要**：本项目需要 JDK 21+。在 Git Bash 下 `./mvnw` 因 MSYS 路径兼容问题无法正常工作，请使用 `bash mvn21.sh <goals>` 替代。

---

## 快速开始

### 1. 启动基础设施

```powershell
# 启动 Redis + Nacos + Zipkin
cd D:\workspace\SmartAssistant
docker-compose up -d

# 可选：启动 Milvus 向量数据库（启用 app.milvus.enabled=true 时需要）
docker compose -f monitoring/docker-compose-milvus.yml up -d

# 验证
docker ps --format "table {{.Names}}\t{{.Status}}"
```

### 1b. 启动 Ollama 本地推理引擎

> ⚠️ **必须操作**：所有 Agent 服务依赖本地 Ollama 模型，启动服务前请确保：

```powershell
# 1. 安装 Ollama（仅首次）
# Windows: 从 https://ollama.com/download 下载安装
# Linux: curl -fsSL https://ollama.com/install.sh | sh

# 2. 下载模型（仅首次）
ollama pull qwen2.5:7b    # 主推理模型

# 3. 启动 Ollama 服务
ollama serve

# 4. 验证模型可用
ollama list
```

### 2. 配置环境变量

```powershell
# 从模板创建配置文件
cd D:\workspace\SmartAssistant
copy .env.example .env
```

关键变量说明：

| 变量 | 说明 | 默认值 |
|------|------|--------|
| `DEEPSEEK_API_KEY` | DeepSeek API 密钥（General/Order/Product 需要） | — |
| `POSTGRES_PASSWORD` | PostgreSQL 密码 | `postgres123` |
| `REDIS_PASSWORD` | Redis 密码 | `redis123` |
| `NACOS_PASSWORD` | Nacos 密码 | `nacos123` |
| `JWT_SECRET` | JWT 签名密钥 | 建议自行生成 |
| `app.data.dir` | 数据存储根目录 | `data` |

> 💡 **混合推理架构**：General/Order/Product Agent 使用 DeepSeek 云端推理（需 `DEEPSEEK_API_KEY`），Router/Consumer 使用 Ollama 本地推理。
> 
> 轻量模型（qwen2.5:3b）通过 Spring AI 委托模式注入，无需额外配置。

### 3. 构建项目

> ⚠️ Git Bash 下 `./mvnw` 因 MSYS 路径兼容问题不可用，请使用 `bash mvn21.sh`。
> PowerShell/cmd 下可使用 `mvnw.cmd`，但需先执行 `set JAVA_HOME=D:\Program Files\Java\jdk-21.0.6+7`。

```powershell
cd D:\workspace\SmartAssistant

# ⭐ 推荐：使用 mvn21.sh 构建（兼容 Git Bash）
bash mvn21.sh install -pl smart-assistant-common -DskipTests
bash mvn21.sh compile -DskipTests

# 全量打包
bash mvn21.sh package -DskipTests -Dmaven.test.skip=true

# 前端（React + TypeScript + TDesign）
cd frontend && npm install && cd ..
```

### 4. 启动服务

#### 方式一：IDE 开发（IntelliJ IDEA 推荐）

1. 打开项目根目录
2. 设置运行配置的环境变量（指向 `.env` 中的值）
3. **必须先清除 `SERVER__PORT` 环境变量**（双下划线会覆盖 `server.port`）：`Remove-Item Env:SERVER__PORT`
4. 按依赖顺序启动：
   - 先启动基础设施：Redis、Nacos（`docker-compose up -d`）
   - 启动 **Embedding Service**（8091）：需先让 BGE ONNX 模型就绪
   - 启动 **Gateway**（8081）→ **User**（8086）→ **Consumer**（8082）→ **Router**（8083）→ **Order**（8085）→ **Product**（8084）→ **General**（8087）
   - 无严格顺序要求，但 Router 依赖 Nacos，Consumer 依赖 Redis

```powershell
# 批量启动（需先修改各服务的端口配置）
cd D:\workspace\SmartAssistant
Remove-Item Env:SERVER__PORT

# 逐个启动（推荐在 IDE 中启动，方便调试）
.\mvnw.cmd spring-boot:run -pl smart-assistant-gateway
```

#### 方式二：前端

```powershell
cd frontend && npm install && npm run dev
```

---

## 详细部署流程

### 数据库初始化

```powershell
# 创建数据库
psql -h 127.0.0.1 -U postgres -c "CREATE DATABASE a2a_system;"

# 安装 pgvector 扩展
psql -h 127.0.0.1 -U postgres -d a2a_system -c "CREATE EXTENSION IF NOT EXISTS vector;"

# 执行初始化脚本
$env:PGPASSWORD='postgres123'; & "C:\Program Files\PostgreSQL\18\bin\psql.exe" -h 127.0.0.1 -U postgres -d a2a_system -f "docs/database/schema.sql"
$env:PGPASSWORD='postgres123'; & "C:\Program Files\PostgreSQL\18\bin\psql.exe" -h 127.0.0.1 -U postgres -d a2a_system -f "docs/database/seed_data.sql"

# ⚠️ chat_messages 相关表已废弃，由 data/users/{userId}/memories/ 替代
# 如需删除旧表，执行：docs/database/cleanup_v20260508.sql
```

### 启用 pg_stat_statements（可选，SQL 性能监控）

```sql
-- 1. 修改 postgresql.conf
-- shared_preload_libraries = 'pg_stat_statements'

-- 2. 重启 PostgreSQL 后执行
CREATE EXTENSION IF NOT EXISTS pg_stat_statements;
```

### 监控栈部署

```powershell
cd monitoring
docker-compose -f docker-compose.yml up -d

# ⭐ 若启用 Milvus 向量数据库，还需启动 Milvus 集群
docker compose -f docker-compose-milvus.yml up -d
```

监控组件：

| 组件 | 访问地址 | 说明 |
|------|---------|------|
| Grafana | http://localhost:3000 | 仪表盘（默认 admin/admin） |
| Prometheus | http://localhost:9090 | 指标查询 |
| Jaeger | http://localhost:16686 | 链路追踪 |
| Loki | http://localhost:3100 | 日志查询 |
| Zipkin | http://localhost:9411 | 追踪兼容层 |

### Docker 容器化部署

项目提供完整的 Docker 容器化方案，支持一键构建部署全部 7 个服务：

```powershell
# 1. 从项目根目录构建
cd D:\workspace\SmartAssistant

# 2. 一键构建并启动所有服务
docker compose -f docker-compose.deploy.yml build
docker compose -f docker-compose.deploy.yml up -d

# 3. 验证
docker compose -f docker-compose.deploy.yml ps
```

各服务独立构建（适用于单独更新某个服务）：

```powershell
docker compose -f docker-compose.deploy.yml build gateway
docker compose -f docker-compose.deploy.yml up -d gateway
```

部署文件说明：

| 文件 | 用途 |
|------|------|
| `Dockerfile` | 多阶段构建（打包 → JRE 运行） |
| `docker-compose.deploy.yml` | 编排全部 7 个服务 + 基础设施 |
| `.dockerignore` | 排除本地配置和构建缓存 |

---

## 配置说明

### 模型文件

系统使用以下模型文件（不随代码提交，需单独放置）：

| 模型 | 用途 | 路径 | 大小 |
|------|------|------|:----:|
| BGE-small-zh-v1.5 | Router Cache T3 语义匹配 | `models/bge-small-zh-v1.5.onnx` | 95 MB |
| BGE-large-zh-v1.5 | Product/Order RAG 检索（384维） | `models/bge-large-zh-v1.5.onnx` | 1.2 GB (FP32) |
| **bge-reranker-v2-m3** | Cross-Encoder 重排序（实验性，默认关闭） | `models/bge-reranker-v2-m3.onnx` | 192 KB + 2.2 GB 权重 |
| **deepseek-r1:7b** (Ollama) | **主推理模型** | Ollama 管理 | 4.7 GB |
| **qwen2.5:7b** (Ollama) | **备选推理模型** | Ollama 管理 | 4.7 GB |

**推理引擎**：所有服务默认使用 Ollama 本地模型（deepseek-r1:7b），通过 Spring AI @Primary ChatModel 注入。无云端 API 依赖。

模型路径通过 `bge.model.path` 配置（支持环境变量 `BGE_MODEL_PATH` 覆盖），首次加载约 4-5 秒，纯本地推理 30-50ms/次。

### 微调管道

项目根目录 `training/` 包含完整的 QLoRA 微调工具集，用于对本地 7B 模型进行参数高效微调：

```
training/
├── app.py               # Streamlit 控制面板
├── finetune_lora.py     # QLoRA 微调主脚本
├── prepare_data.py      # 数据准备与生成
├── export_gguf.py       # 导出 GGUF + 注册 Ollama
├── test_model.py        # 快速测试
└── sample_data/         # 示例数据
```

使用详见 `training/README.md`。

### 配置层级（优先级从高到低）

1. **命令行参数**：`--server.port=8082`
2. **OS 环境变量**：`OLLAMA_BASE_URL=http://server:11434`（默认 localhost:11434）
3. **`.env` 文件**：项目根目录（自动加载）
4. **`application.yml`**：各服务的配置文件
5. **默认值**：`${VAR:defaultValue}` 语法

> 💡 **混合推理架构**：General/Order/Product 使用 DeepSeek 云端（需配置 `DEEPSEEK_API_KEY`），Router/Consumer 使用 Ollama 本地。

### 优雅关闭

所有服务已配置优雅关闭（2026-05-11），确保服务停止时处理中的请求完成后再退出：

```yaml
server:
  shutdown: graceful
spring:
  lifecycle:
    timeout-per-shutdown-phase: "30s"
```

无需额外操作，`kill` 或 `Ctrl+C` 时自动生效。

### 关键配置目录

```
smart-assistant-{service}/src/main/resources/
├── application.yml           # 服务配置
├── logback-spring.xml        # 日志配置
├── prompts/                  # 系统提示词（外部化，改提示词无需重新编译）
│   ├── travel-system-prompt.txt
│   ├── food-system-prompt.txt
│   └── general-system-prompt.txt
├── skills/
│   └── skills.yml            # ⭐ P4 可配置技能仓库（9 个技能）
└── config/
    └── mcp-table-whitelist.yml  # MCP 表访问白名单
```

### P4 调度配置

```yaml
# application.yml — 默认关闭，启用后 Router 支持异步任务队列
scheduler:
  enabled: false                # 设为 true 启用
  worker-count: 4               # Worker 线程数
  poll-timeout: 10              # BRPOP 轮询超时（秒）
```

### 系统提示词外部化

各 Agent 服务的系统提示词已从 Java 代码中提取到独立的资源文件，修改无需重新编译：

```
smart-assistant-{service}/src/main/resources/prompts/
├── order-system-prompt.txt        # 订单客服助手
├── product-system-prompt.txt      # 商品咨询助手
└── general-system-prompt.txt      # 通用对话助手
```

加载方式：`@Value("classpath:prompts/{service}-system-prompt.txt")`，IO 异常时自动降级为默认提示词，不影响服务启动。

### Service 包结构

```
smart-assistant-router/.../service/
├── core/          RouterService, SmartRoutingService, ReflectionService(含 checkCriteria 验收检查), DegradationService(异常率阈值降级)
├── agent/         AgentCallerService(含 RestTemplate 超时+CircuitBreaker), AgentDiscoveryService, AgentHealthChecker, AgentVersionNegotiator
├── cache/         SemanticRouteCacheService, RoutingDecisionStorageService
├── infrastructure/ DistributedTracingService
├── extraction/    KeywordExtractionService
├── rag/           RouterRagService(含摘要回指: extractEntities+generateBackReferences)
├── evaluation/    EntityNormalizer, SlotStateMachine, ClarificationService,
│                  IntentEvaluationService, IntentGuidedQueryRewriter 🆕
├── monitoring/    RouterMetricsCollector, NewMetricsCollector(含 ErrorType+延迟分位数)
├── experience/    ExperienceService(含 BGE 语义合并 findSimilarByBGE)
├── tool/          RoutingToolChecker
├── skill/         SkillRepository, SkillDefinition
├── taskanalysis/  TaskAnalysisService
├── checkpoint/    GraphCheckpointService 🆕 (Redis 持久化 + 断点恢复)
└── model/         IntentGraph(条件边/循环/HITL), GraphState(Reducer 语义) 🆕

smart-assistant-common/.../service/ (新增大包路径)
├── rag/eval/      RetrievalMetrics, HallucinationDetector, RAGEvaluator, RAGEvaluationResult 🆕
└── eval/          AgentEvaluationResult, EvaluationReportService 🆕

smart-assistant-food/.../service/
├── core/          ABTestService, HybridRecommendationService, ReviewEmbeddingInitializer
├── search/        RestaurantReviewSearchService
├── infrastructure/ DistributedTracingService
├── agent/         StreamingFoodAgentService
└── monitoring/    FoodMetricsCollector

smart-assistant-travel/.../service/
├── rag/           TravelRagService, TravelNoteService, TravelNoteMatchService, TravelNoteRankingService,
│                  SemanticChunker, EmbeddingService, RecallService, AttractionVectorService
├── data/          DatabaseAttractionService, AttractionDataImportService, AmapPoiSyncService,
│                  DataQualityValidator
├── infrastructure/ DistributedTracingService
├── agent/         McpAgentService, StreamingTravelAgentService
└── monitoring/    TravelMetricsCollector
```

### 查看日志

所有服务的日志统一输出到 `logs/` 目录，格式为 `{spring.application.name}.log`：

```powershell
# 应用日志（纯净 Spring Boot 输出，推荐优先查看）
type logs\router-service.log -Tail 50
type logs\order-service.log -Tail 50
type logs\product-service.log -Tail 50
type logs\general-agent-service.log -Tail 50
type logs\consumer-service.log -Tail 50
type logs\api-gateway.log -Tail 50
type logs\user-service.log -Tail 50

# 标准输出日志（含启动横幅等）
type logs\Router-stdout.log -Tail 30
type logs\Order-stdout.log -Tail 30
type logs\Product-stdout.log -Tail 30
```

### ⚠️ 密码配置强制要求

以下密码**必须通过环境变量设置**，未设置时服务启动即报错：

| 变量 | 说明 |
|------|------|
| `JWT_SECRET` | JWT 签名密钥 |
| `POSTGRES_PASSWORD` | PostgreSQL 密码（默认 postgres123） |
| `REDIS_PASSWORD` | Redis 密码（默认 redis123） |
| `NACOS_PASSWORD` | Nacos 密码（默认 nacos123） |

### 用户数据存储

所有用户数据存储在 `data/users/{userId}/` 目录下，不再依赖数据库：

```
data/users/{userId}/
├── preferences.json            # 用户偏好（权重、意图分布）
├── order-memory.md             # Order Agent 记忆（行程偏好、座位偏好）
├── product-memory.md           # Product Agent 记忆（品类偏好、价格区间）
├── general-memory.md           # General Agent 记忆（回复风格、常用单位）
└── memories/
    ├── 2026-05-08_session_abc.md   # 增量追加记忆文件
    └── 2026-05-08_session_def.md
```

**Agent 独立记忆**：每个 Agent 拥有用户粒度的键值记忆文件（Markdown 格式），记录可复用的用户偏好和习惯。

```markdown
# Order Agent 用户偏好

- preferWindowSeat: 靠窗
- frequentRoute: 北京→上海
- preferPaymentMethod: 微信支付
```

Agent 通过 `savePreference(userId, key, value)` 写入，`recallMemories(userId)` 读取。
系统在每次处理用户问题前自动将记忆注入 prompt 上下文，提供个性化回复。
记忆文件在首次写入时自动创建，无需手动初始化。

**对话摘要持久化（方案 A）**：SmartReActAgent 每次 Context 压缩后，自动将 9 段式摘要嵌入为 BGE 向量并存入 Milvus `conversation_summaries` Collection（IVF_FLAT + COSINE 索引）。支持按用户语义检索历史摘要。通过 `withSummaryStore()` 注入，不可用时静默降级。

---记忆文件采用 **session 优先的增量追加**模式。同 session 始终追加到同一文件（跨天不换文件），文件名为首次创建的日期：

```yaml
---
created_at: 1746680000000
session: session_abc
turn_range: 3-5
entries: 3
---

> Turn 3 | narrative | intent: 景点查询

用户咨询了景点信息，系统推荐了故宫和天坛...

---

> Turn 4 | raw | intent: 美食推荐

用户：有川菜推荐吗？
助手：眉州东坡、辣婆婆...

---

> Turn 5 | narrative | intent: 旅游规划

用户继续咨询三日游行程安排，系统制定了详细规划...
```

每个条目通过 `> Turn {n} | {format} | intent: {intentTag}` 标记轮次和格式：
- **`narrative`** — LLM 第三人称叙事摘要（轮数≥3 且内容≥1000 字符时触发）
- **`raw`** — 原文保存（内容不足摘要阈值）
- 超 8000 字符时内容被安全截断，截断部分以 `---` 分隔直接原文追加

### 全局纠错记录

Agent 的用户修正记录存储在 `data/corrections/` 目录，所有用户共享，用于避免重复错误：

```
data/
├── users/{userId}/memories/       # 用户私人记忆（增量追加）
└── corrections/
    ├── travel.md                   # 出行修正记录（全局共享）
    ├── food.md                     # 美食修正记录
    └── general.md                  # 通用修正记录
```

修正记录格式：

```markdown
# Travel Agent 用户修正记录

## 2026-05-11

> 主题: 故宫开放时间
> 错误: 故宫8:00开门
> 正确: 故宫冬季8:30开门，夏季8:00开门
> 来源: userId=123
```

各 Agent 的 system prompt 强制要求：回答事实性问题前先调用 `queryCorrections(topic)` 工具，有匹配修正时以修正信息为准。



---

## 请求排队

系统基于压测数据实现了三层请求排队流控机制，在 LLM 并发槽位占满时自动排队，通过 SSE 实时通知用户排队状态。

### 三层流控

| 层级 | 机制 | 配置 | 效果 |
|:----|:-----|:-----|:-----|
| **L1** | Semaphore 全局并发控制（公平模式） | `chat.queue.max-concurrent` (默认 5) | 限制整体 LLM 并发数 |
| **L2** | 会话级并发控制（ConcurrentHashMap + AtomicInteger） | `queue.session-max-concurrency` (默认 1) | 同一 sessionId 最多 1 个请求在处理，防刷屏 |
| **L3** | PriorityBlockingQueue 优先级调度 | `queue.priority-enabled` (默认 false) | VIP 用户请求优先获取槽位，priority 越低越优先 |

### 工作流程

```
用户发送消息
  │
  ├→ L2: 检查会话并发数(已占用? → 排队)
  │
  ├→ L1: [有槽位] event: processing → 正常 SSE 流 → event: done
  │
  └→ [无槽位] event: queued {position: 3, estimatedWaitMs: 15000}
              │
              ├→ L3: PriorityBlockingQueue 自动排序
              ├→ Semaphore.tryAcquire(60s 超时) 阻塞等待
              │
              ├→ [获取到槽位] event: processing → 正常 SSE 流 → event: done
              └→ [超时] event: timeout → 连接关闭

处理完成后自动释放 L1 槽位 + L2 会话槽位
```

### SSE 事件类型

| 事件 | 说明 | 数据 |
|------|------|------|
| `queued` | 进入排队 | `{position, estimatedWaitMs}` |
| `processing` | 槽位分配，开始处理 | `{}` |
| `timeout` | 排队超时 | `{content: "排队超时，请稍后重试"}` |
| `waiting` | 等待路由决策 | `{content: "正在分析意图..."}` |
| `thinking` | AI 思考过程 | `{type: "thinking", content: "..."}` |
| `tool_call` | 工具调用 | `{type: "tool_call", name, input}` |
| `tool_result` | 工具执行结果 | `{type: "tool_result", content}` |
| `response` | 最终回复 | `{type: "response", content}` |
| `done` | 完成信号 | `{type: "done"}` |

### 配置项

在 `smart-assistant-consumer/application.yml` 中：

```yaml
chat:
  queue:
    max-concurrent: 5          # 最大并发 LLM 请求数（基于压测数据）
    max-queue-size: 50         # 最大排队长度
    queue-timeout-ms: 60000    # 排队等待超时（毫秒）
```

### 前端表现

排队中时，用户消息气泡下方显示：

```
⏳ 排队中，前面还有 3 人，预计等待 15 秒
```

槽位就绪后自动开始处理，用户无感知。

---

## API 文档

### 认证接口

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/assistant/api/auth/login` | 用户登录，返回 JWT Token |
| POST | `/assistant/api/auth/register` | 用户注册 |
| GET | `/assistant/api/auth/me` | 获取当前用户信息 |

### 对话接口

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/assistant/api/math/chat` | 发送消息（非流式，通过 Router 路由到对应 Agent） |
| POST | `/assistant/api/data/query` | ⭐ 数据查询（仅 ADMIN，独立端点，不混入对话流） |
| WebSocket | `/assistant/ws/conversation` | WebSocket 实时对话 |

### 用户反馈接口

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/feedback` | 提交用户对 Agent 回复的评价（like/dislike）。请求体：`{userId, sessionId, question, response, rating, reason}` |

### 通用助手工具（General Agent）

| 工具 | 说明 | 依赖 |
|------|------|------|
| `calculate(expression)` | 数学计算 | — |
| `convertTemperature/Length/Weight` | 单位转换 | — |
| `getHotNews()` | 网络热点新闻 | — |
| `searchWeb(query)` | 联网搜索 | — |
| `convertCurrency(value, from, to)` | 货币汇率转换 | 实时汇率 API |
| `executeScript(script)` | **沙箱保护**的多步计算脚本执行（关键字黑名单/资源限制/超时熔断） | — |
| `queryWeather(city)` | 天气查询 | — |
| `savePreference(userId, key, value)` | 保存用户偏好（Order/Product/General 三 Agent 均有） | AgentMemoryService |
| `recallMemories(userId)` | 获取用户保存的所有偏好记忆 | AgentMemoryService |

### Agent 控制接口

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/order/agent/process` | Order Agent 直调（支持 userId 可选，自动注入记忆和后台提取偏好） |
| POST | `/product/chat/sync` | Product Agent 同步对话 |
| GET | `/product/stream/chat` | Product Agent SSE 流式对话 |

### 路由接口

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/assistant/api/router/route` | 智能路由（意图识别 + Agent 调度） |
| GET | `/assistant/api/router/tools/health` | 工具健康检查（查看各 Agent 工具注册状态） |
| GET | `/assistant/api/router/events/{requestId}` | Agent 执行轨迹查询（状态机事件链） |

### 推荐接口

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/recommend` | 🎯 P3 跨模块推荐。请求体：`{userId, productCode, maxResults}`。支持图谱推荐+协同过滤+热门兜底三策略 |
| GET | `/api/recommend/health` | 推荐服务健康检查 |

### 图谱接口

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/product/list` | 全量商品列表（含品类/品牌） |
| GET | `/api/product/{code}/recommend` | 商品图谱关联推荐 |
| GET | `/api/order/user/{userId}/products` | 用户购买历史（商品编码列表） |
| GET | `/api/order/user/{userId}/orders` | 用户订单摘要 |

### 智能纠错工具（所有 Agent）

| 工具 | 适用 Agent | 说明 |
|------|-----------|------|
| `queryCorrections(topic)` | Travel / Food / General | 查询历史修正记录。Agent 在回答事实性问题前自动调用，检查是否有用户反馈过的修正信息并优先采用 |

### 健康检查

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/actuator/health` | 服务健康状态 |
| GET | `/actuator/prometheus` | Prometheus 指标 |
| GET | `/actuator/info` | 服务信息 |

### 离线评测

| 工具 | 路径 | 说明 |
|------|------|------|
| 黄金测试集 | `docs/eval/sample-test-set.json` | 14 个测试用例覆盖 Order/Product/General 三个 Agent。运行：`java com.example.smartassistant.common.eval.GoldenTestRunner docs/eval/sample-test-set.json` |
| 对抗式测试集 | `docs/eval/adversarial-test-set.json` | 22 个对抗式用例覆盖 5 类场景：无答案问题(3)、Prompt 注入(3)、权限越界(2)、同义改写鲁棒性(5)、否定/错别字/多意图(6)。预期含 INSUFFICIENT_EVIDENCE 拒答检查 |
| ⭐ **RAG 评测框架** | `common.rag.eval.*` | `RetrievalMetrics`(Recall@K/Precision@K/MRR/nDCG) + `HallucinationDetector`(4层规则检测) + `RAGEvaluator`(编排器) + `RAGEvaluationResult`(综合评分+批量报告)。黄金测试集: `eval-test-suite.json`(4 RAG 用例) |
| ⭐ **Agent 评测框架** | `common.eval.*` | `AgentEvaluationResult`(Builder,5维度评分) + `EvaluationReportService`(黄金测试集管理+批量评测)。黄金测试集: `eval-test-suite.json`(6 Agent 用例) |
| 🚪 **评测闭环门禁 (CI)** | `common.eval.gate.*` | `EvalGateConfig`(绝对阈值) + `EvalBaseline`(基线) + `EvalGate`(判定器) + `EvalReportExporter`(报告导出) 四件套 + `GoldenSuiteEvalGate`(编排器，含 `main` 可独立重生成基线)。**双保险**：绝对阈值防断崖 + 基线回归(±0.05)防渐变。`.github/workflows/eval-gate.yml` 在 push/PR 到 main 触发 compile-all + 跑黄金集，首次自举生成 `eval-baseline.json`，门禁不通过则 CI 失败阻断合入 |

#### Agent 评测体系四缺口补齐（Gap①~④）

基于《Agent 评测体系：从 demo 到生产的完整拆解》(Anthropic + 阿里工程化体系) 对照分析，补齐 SA 评测体系在"攻"与"深度"层面的 4 项系统性缺口（"守"层基线回归门禁已达目标不改动）。完整设计见 [`docs/eval-system-gaps-design.md`](docs/eval-system-gaps-design.md)。

| 缺口 | 新增组件（`common.eval`） | 关键设计 |
|------|--------------------------|----------|
| **① Trial / pass^k** | `TrialResult` / `PassKCalculator` / `TrialRunner` | 多次独立运行封装 `Supplier<AgentEvaluationResult>`，聚合 `pass@k`(≥1次成) 与 `pass^k`(全成)；`passPowerK(0.6, 5)≈0.078` |
| **② Grader 瀑布流** | `grader/` 子包：`Grader` / `GraderResult` / `RuleGrader` / `LlmGrader` / `GraderWaterfall` | 规则(硬条件)→LLM-Judge(语义评分)；`GraderResult` 含 `partialCredit`(0~1)；`LlmGrader` 内置 3 大偏差防护（位置/长度/自我偏好） |
| **③ 人工路由** | `HumanReviewTask` / `HumanReviewStore` / `InMemoryHumanReviewStore` / `HumanReviewRouter` | 2% 随机抽检 + 边界分数 `[0.4,0.7]` 强制升级 + LLM/规则分歧升级至人工 |
| **④ 根因分析** | `RootCauseTag` / `FailureSignature` / `RootCauseAnalysis` / `RootCauseAnalyzer` | 5 步（证据→收敛→诊断→定责→落盘）+ 规则指纹聚类定责，输出可回流修复工单 |
| **集成编排** | `EvalPipeline` | Trial→Grader 瀑布→pass^k→人工路由→根因分析串联；`ChatModel` 可空离线安全 |

**门禁接入**：`GoldenSuiteEvalGate.run()` 新增可注入 `TrialExecutor` 的重载——注入运行器且 `enableAgentGate && agentTrialCount>1` 时走 `EvalPipeline`，以 `pass^k ≥ minAgentPassKRate`(默认 0.8) 作为 Agent 单用例稳定性门禁依据；`AgentEvaluationResult` 新增可空 `overridePassed` 供门禁映射（默认走原逻辑，零改动 `EvalGate`）。

**测试**：`EvalGapsDesignTest`(10)、`GoldenSuiteEvalGateTrialTest`(3)、`AgentEvaluationResultOverrideTest`(3) 覆盖四缺口行为 + pass^k 门禁阻断 + 长度偏差封顶。

---

## 常见问题

### Q: 服务启动绑定到异常端口（如 63807）

检查是否设置了 `SERVER__PORT` 环境变量（注意是双下划线）。该变量会覆盖 `server.port` 配置，需要在 shell 中清除：

```powershell
# PowerShell
Remove-Item Env:SERVER__PORT

# CMD
set SERVER__PORT=
```

检查 `.env` 文件是否存在且包含正确的 `DEEPSEEK_API_KEY`。如果以 IDE 启动，需在运行配置中设置环境变量。


### Q: 中文请求返回"一串问号"

Windows 终端（PowerShell/cmd）默认使用 GBK 编码发送 JSON，服务端只接收 UTF-8。PowerShell 中需强制 UTF-8 编码：

```powershell
$body = @{ message='你的中文问题' } | ConvertTo-Json
$utf8Bytes = [System.Text.Encoding]::UTF8.GetBytes($body)
Invoke-RestMethod -Uri 'http://localhost:8081/assistant/api/math/chat' -Method Post -Body $utf8Bytes -ContentType 'application/json; charset=utf-8' -Headers $headers
```

cmd 中先执行 `chcp 65001` 切换到 UTF-8 代码页。


### Q: 编译报错"编码 UTF-8 的不可映射字符"

这是由于使用 PowerShell 的 `Set-Content` 命令操作含中文的 Java 文件时，默认使用了系统 ANSI 编码（GBK），导致中文字符损坏。

**修复方法**：使用 `git checkout -- <file>` 恢复原始文件，然后使用以下命令之一操作：
- Bash: `sed -i`（正确处理 UTF-8）
- PowerShell: `[System.IO.File]::WriteAllBytes($path, [System.Text.Encoding]::UTF8.GetBytes($content))`

### Q: Consumer 与 Router 边界不清如何处理？

2026-05-11 已重构：Consumer 不再将 JSON Prompt 塞入 `question` 字段，改为通过独立字段传递元数据（`userProfile`、`intentTag`、`requestId`、`sessionId`）。Router 不再需要 `extractRequestId()`/`extractThreadId()` 反解析。数据查询功能拆分为独立 `DataQueryController`。

### Q: 模型切换

系统支持**双推理通道**：
- **DeepSeek 云端**（通用/订单/商品 Agent）：修改各服务 `application.yml` 中 `spring.ai.deepseek.chat.options.model`
- **Ollama 本地**（Router/Consumer/轻量通道）：修改 `spring.ai.ollama.chat.options.model`

各通道独立配置，互不影响。

### Q: DeepSeek API Key 配置

General/Order/Product 服务通过 Spring AI 自动配置读取 `spring.ai.deepseek.api-key`，可以通过环境变量 `DEEPSEEK_API_KEY` 或 `.env` 文件设置。Router/Consumer 服务无需此配置，仅使用 Ollama 本地推理。

Nacos 默认开启了认证（`NACOS_AUTH_ENABLE=true`），初始化 Nacos 后需要先通过 Web 控制台（http://localhost:8848/nacos）使用默认账号 `nacos` / `nacos123` 登录。所有服务的 `application.yml` 已配置认证信息。

### Q: Agent 回答明显错误如何修正？

直接在对话中指出错误即可，例如"不对，故宫是8:30开门"或"你说错了"。目前修正需要手动追加到 `data/corrections/{agent}.md` 文件。参考文件头部注释的格式添加条目后，下次 Agent 回答相关问题时将自动参考修正记录。

计划在后续版本中支持用户通过对话自动记录修正。

### Q: Redis 连接被拒绝

Redis 默认启用了密码认证（`redis123`），所有服务已配置 `password: ${REDIS_PASSWORD:redis123}`。如需修改密码，需要同时更新 `docker-compose.yml` 和所有服务的 `application.yml`。

### Q: PostgreSQL 连接失败

确认 PostgreSQL 已启动且 `a2a_system` 数据库已存在。默认连接配置在 `smart-assistant-user` 和 `smart-assistant-consumer` 的 `application.yml` 中。

### Q: 编译报错 "Unable to rename"

这是由于 Windows 文件锁导致的，常见于之前有 Java 进程未完全退出。执行：

```powershell
taskkill /F /IM java.exe
# 然后重新编译
.\mvnw.cmd clean compile -DskipTests
```

### Q: 前端页面空白

确认前端服务已启动（端口 3001），检查 Vite 代理配置是否正确。前端通过 `/api` 前缀代理到 Gateway 的 8081 端口。

### Q: 启用 Nacos 认证后现有服务连接不上

Nacos 认证只在初始启动时生效。如果已存在的 Nacos 实例需要重启才能启用认证：

```powershell
docker-compose restart nacos
```

---

## 开发指南

### 模块依赖关系

```
smart-assistant-common (核心工具：分词器、SQL 校验器、Dotenv、修正记录服务、@Tool 日志切面、RAG评测框架、Agent评测框架)
    ↑          ↑          ↑          ↑
Gateway   Consumer    Router     Travel / Food / User / General
(无common)   (common)   (common)   (common)
```

### 添加新的 Agent

1. 在 `smart-assistant-{agent}` 模块中实现 `@Tool` 方法
2. 配置 `mcp-table-whitelist.yml` 中的表访问权限
3. 在 `application.yml` 中配置基础 Nacos 注册信息
4. Agent 启动后会自动通过 Nacos 注册到 Router 的服务发现列表
5. 在 Nacos UI 中创建配置 `{serviceName}-metadata` (Group: `AGENT_META`) 设置 keywords/priority
6. 后续修改 Agent 职责只需改 Nacos Config，无需重新部署

### 运行测试

```powershell
# 全量测试
.\mvnw.cmd test -DskipTests=false

# 指定模块
.\mvnw.cmd test -pl smart-assistant-gateway

# 指定测试类
.\mvnw.cmd test -pl smart-assistant-common -Dtest=SqlSecurityValidatorTest
```

当前测试覆盖：

| 模块 | 测试数 | 覆盖内容 |
|------|--------|---------|
| common | **257** | SQL 安全校验器、中文分词器...**AgentMemoryService(6用例)**、**TraceSpan(3用例)**、**GoldenTestRunner(5用例)**、**⭐ RAG评测框架(22用例:RetrievalMetrics/HallucinationDetector/RAGEvaluator/RAGEvaluationResult)**、**⭐ Agent评测框架(5用例:AgentEvaluationResult)** |
| gateway | 33 | JWT 工具、白名单过滤、Filter 认证、Filter 集成测试 |
| user | 9 | JWT 服务 |
| consumer | 25 | 对话叙事摘要、文档沉淀服务、DataGifTool、Chat 集成测试 |
| router | **95+** | Agent 调用、语义缓存...**IntentRetriever(1用例)**、**ExperienceValidator(5用例)**、**HandoffCommand(3用例)**、**KeywordFastRoute(12基准测试:7.3μs/次+多意图检测)**、**BadCaseMiner(6决策逻辑测试)**、**RouterServiceEndToEnd(9全链路集成测试)**、**⭐ GraphCheckpoint(含checkpoint/条件边/循环)** |
| product | 11 | **ProductGraph(6图谱测试:15节点/36边/1.947μs)**、**RAG质量评分(5评分测试)** |
| order | 11 | **ApprovalStateMachine(11状态机测试:0.87μs/次/200线程并发安全)** |
| recommend | 5 | **RecommendEngine(5推荐引擎集成测试:图谱+协同过滤+去重+性能0.120ms)** |
| general | **44** | 数学计算、温度/长度/重量/货币转换、边界条件、**ScriptSandbox 沙箱(14测试)**、**⭐ 4处工具返回值结构化修复** |
| **总计** | **490+** | **含 LangGraph 对标全特性测试，0 Failures** |

> ⚠️ **CI 已接入路由核心回归卡点**：`.github/workflows/eval-gate.yml` 新增 `router-e2e-gate` 作业（纯 Mockito 注入、零基础设施依赖），将 `RouterServiceEndToEndTest` 设为 **required status check**，防止该类红灯被长期掩盖。本地复跑：
> `bash mvn21.sh -pl smart-assistant-router -am test -Dtest=RouterServiceEndToEndTest -Dsurefire.failIfNoSpecifiedTests=false`

---

## 历史迭代日志

> 2026-07-08 起的阶段性改造记录（近期改进、MCP 工具扩展、tool-registry 治理、P0 阻断项修复等）
> 已迁移至 [docs/changelog.md](docs/changelog.md)，避免主文档过长。

---

## 许可证

[MIT License](LICENSE)

Copyright (c) 2025-2026 SmartAssistant Project
