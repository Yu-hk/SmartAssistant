...
/**
 * Travel Service 监控指标收集器
 * 记录请求延迟、成功率等关键指标
 */
@Component
public class TravelMetricsCollector {
    ...
    public TravelMetricsCollector(MeterRegistry meterRegistry) {

        // 初始化计数器
        this.totalRequestsCounter = Counter.builder("a2a_travel_requests_total")
                .description("Total number of requests received by Travel Service")
                .tag("service", "travel-service")
                .register(meterRegistry);
        
        this.successfulRequestsCounter = Counter.builder("a2a_travel_requests_success_total")
                .description("Total number of successful requests")
                .tag("service", "travel-service")
                .register(meterRegistry);
        
        this.failedRequestsCounter = Counter.builder("a2a_travel_requests_failed_total")
                .description("Total number of failed requests")
                .tag("service", "travel-service")
                .register(meterRegistry);
        
        // 初始化计时器
        this.requestLatencyTimer = Timer.builder("a2a_travel_request_latency")
                .description("Request latency in milliseconds")
                .tag("service", "travel-service")
                .register(meterRegistry);
        
        this.recommendationLatencyTimer = Timer.builder("a2a_travel_recommendation_latency")
                .description("Recommendation generation latency in milliseconds")
                .tag("service", "travel-service")
                .register(meterRegistry);
        
        // 注册 Gauge
        meterRegistry.gauge("a2a_travel_active_recommendations", 
