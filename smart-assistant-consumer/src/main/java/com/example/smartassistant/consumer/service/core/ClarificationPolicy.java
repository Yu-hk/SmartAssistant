...
public final class ClarificationPolicy {
    ...
    public record Field(String key, String label, String type, String unit, String value,
                        String min, String max, int decimals, int minLength, int maxLength, String hint) { }
    private static final Map<String, Field> FIELDS = Map.ofEntries(
            Map.entry("weight", new Field("weight", "重量上限", "number", "公斤", "", "0.001", "1000", 3, 1, 20, "请填写公斤数，最多 3 位小数")),
            Map.entry("budget", new Field("budget", "预算上限", "number", "元", "", "0.01", "100000000", 2, 1, 20, "请填写人民币预算，最多 2 位小数")),
            Map.entry("quantity", new Field("quantity", "购买数量", "number", "件", "", "1", "10000", 0, 1, 10, "请填写整数件数")),
            Map.entry("city", new Field("city", "城市", "text", "", "", null, null, 0, 1, 40, "请填写城市名称")),
            Map.entry("orderNumber", new Field("orderNumber", "订单号", "text", "", "", null, null, 0, 1, 69, "以 ORD- 或 BULK- 开头的订单编号")),
            Map.entry("product", new Field("product", "商品名称或类型", "text", "", "", null, null, 0, 1, 100, "请填写商品名称或类型")),
            Map.entry("recipientName", new Field("recipientName", "收货人姓名", "text", "", "", null, null, 0, 1, 40, "请填写收货人姓名")),
            Map.entry("recipientPhone", new Field("recipientPhone", "联系电话", "text", "", "", null, null, 0, 1, 11, "请填写 11 位中国大陆手机号")),
            Map.entry("shippingAddress", new Field("shippingAddress", "收货地址", "text", "", "", null, null, 0,
                    OrderShippingAddressPolicy.MIN_LENGTH, OrderShippingAddressPolicy.MAX_LENGTH,
                    OrderShippingAddressPolicy.HINT)),
            Map.entry("reason", new Field("reason", "具体原因", "text", "", "", null, null, 0, 1, 200, "请填写具体原因")),
            Map.entry("afterSalesType", new Field("afterSalesType", "售后类型", "text", "", "", null, null, 0, 1, 4, "填写：退货、换货或维修")));
    ...
    public static String reply(List<String> keys, Map<String, String> values) {
        ...
        for (String key : keys) {
            ...
            if (value.length() < field.minLength()) throw invalid();
            ...
            } else if ("shippingAddress".equals(key)) {
                if (!OrderShippingAddressPolicy.usable(value)
