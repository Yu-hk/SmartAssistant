 ...
 * 轻量 LLM 推理通道配置。
 * <p>
 * 基于 {@link ChatModel} 委托模式实现，注入 Spring AI 自动配置的 DeepSeek API
 * 模型实例，并通过请求级 {@link ChatOptions} 覆盖默认参数（低温度 +
 * 低温度），实现轻量调用通道。用于：对话摘要、偏好提取、缓存改写、关键词提取、意图识别等辅助任务。
 * <p>
 * 采用委托而非新建模型实例，避免重复构造 ToolCallingManager、
...
public class LightChatModelConfig {
    ...
    public ChatModel lightChatModel(
            ...
            @Value("${consumer.light-model.name:deepseek-v4-flash}") String model,
    ...
    /**
     * 委托 ChatModel 实现。将所有 call/stream 请求转发给底层 API ChatModel，
