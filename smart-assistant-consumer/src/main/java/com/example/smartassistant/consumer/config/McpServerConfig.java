...
public class McpServerConfig {
    ...
    private final RefreshableMcpTableWhitelistConfig whitelistConfig;
    ...
    public List<Map<String, Object>> executeQuery(
        ...
        if (!result.isValid()) {
            log.warn("[MCP Tool - Consumer] ❌ 安全校验未通过: {}", result.getReason());
            log.warn("[MCP Tool - Consumer]    SQL: {}", sql);
            throw new IllegalArgumentException(result.getReason());
        }

        log.debug("[MCP Tool - Consumer] ✅ 安全校验通过，允许的表: {}", allowedTables);

        try {
            List<Map<String, Object>> queryResult = jdbcTemplate.queryForList(sql);
            log.info("[MCP Tool - Consumer] ✅ 查询成功，返回 {} 行", queryResult.size());
            return queryResult;
        } catch (Exception e) {
