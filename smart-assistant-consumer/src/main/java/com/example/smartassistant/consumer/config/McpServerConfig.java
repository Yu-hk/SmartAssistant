...
/**
 * MCP Server 配置 - Consumer Service
...
public class McpServerConfig {
    ...
    private final RefreshableMcpTableWhitelistConfig whitelistConfig;  // ⭐ 使用可刷新配置
    
    public McpServerConfig(JdbcTemplate jdbcTemplate, RefreshableMcpTableWhitelistConfig whitelistConfig) {
     ...
     */
    @Tool(description = "执行只读 SQL 查询（仅支持 SELECT 语句，仅限 consumer 相关表）")
    public List<Map<String, Object>> executeQuery(
        @ToolParam(description = "SQL 查询语句（仅支持 SELECT）", required = true) String sql
    ) {
        log.info("[MCP Tool - Consumer] ⭐ Agent 生成的 SQL: {}", sql);
        
        // ⭐ 安全检查1：只允许 SELECT 语句
        String trimmedSql = sql.trim().toUpperCase();
        if (!trimmedSql.startsWith("SELECT")) {
            log.warn("[MCP Tool - Consumer] ❌ 拒绝非 SELECT 语句");
        ...
        for (String keyword : dangerousKeywords) {
            if (trimmedSql.contains(keyword)) {
                log.warn("[MCP Tool - Consumer] ❌ 拒绝危险操作");
                log.warn("[MCP Tool - Consumer]    SQL: {}", sql);
                log.warn("[MCP Tool - Consumer]    包含危险关键词: {}", keyword.trim());
                throw new IllegalArgumentException("禁止执行危险操作");
            }
        }
        
        // ⭐ 安全检查3：限制可访问的表（从配置文件读取白名单）
        String serviceName = "consumer";  // 当前服务名称
        ...
        for (String table : whitelistConfig.getForbiddenTables(serviceName)) {
            if (lowerSql.contains(table.toLowerCase())) {
                log.warn("[MCP Tool - Consumer] ❌ 拒绝访问未授权的表: {}", table);
                log.warn("[MCP Tool - Consumer]    SQL: {}", sql);
        ...
        if (!hasAllowedTable) {
            log.warn("[MCP Tool - Consumer] ⚠️  SQL 未包含任何允许的表");
            log.warn("[MCP Tool - Consumer]    SQL: {}", sql);
            log.warn("[MCP Tool - Consumer]    允许的表: {}", whitelistConfig.getAllowedTables(serviceName));
            throw new IllegalArgumentException("只能查询 consumer 相关的表");
        }
        
        try {
            List<Map<String, Object>> result = jdbcTemplate.queryForList(sql);
            log.info("[MCP Tool - Consumer] ✅ 查询成功，返回 {} 行", result.size());
            return result;
        } catch (Exception e) {
            log.error("[MCP Tool - Consumer] ❌ SQL 执行失败: {}", e.getMessage());
            throw new RuntimeException("SQL 执行失败: " + e.getMessage(), e);
        }
    }
    
    /**
     * 获取数据库表结构（辅助工具 - 仅返回 consumer 相关表）
     * 当 Agent 不确定表结构时，可以先调用此工具
     */
    @Tool(description = "获取 consumer 相关表的元数据信息")
    public List<Map<String, Object>> getTableSchema() {
        log.info("[MCP Tool - Consumer] 执行 getTableSchema");
        ...
        if (allowedTables.isEmpty()) {
            log.warn("[MCP Tool - Consumer] ⚠️  白名单为空，无法查询表结构");
            return List.of();
        }
