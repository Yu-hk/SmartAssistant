...
public class RouterClient {
    ...
    public String callRouter(String question, String userId, String sessionId, String requestId) {
        ...
        try {
            ...
            if (mdcRequestId != null) {
                headers.set("X-Request-Id", mdcRequestId);
            }
            if (threadId != null) {
                headers.set("X-Thread-Id", threadId);
            }

            // ⭐ 设置 JWT Token (仅使用用户 Token,不再生成匿名 Token)
            try {
                String authToken = null;
                
                // 尝试从当前请求中获取用户的 JWT Token
                ...
                if (attributes != null) {
                    ...
                    if (authHeader != null && authHeader.startsWith("Bearer ")) {
                        ...
                        log.info("[RouterClient] ✅ 使用用户 JWT Token");
                    }
                }
                
                // 设置 Authorization Header (如果没有 Token 则不设置)
                if (authToken != null) {
                    headers.set("Authorization", authToken);
                } else {
                    log.debug("[RouterClient] ℹ️ 未找到 JWT Token,将使用公开接口或返回 403");
                }
                
            } catch (Exception e) {
                log.error("[RouterClient] ❌ 设置 JWT Token 失败: {}", e.getMessage(), e);
            }
            ...
            if (response.getStatusCode() == HttpStatus.OK && response.getBody() != null) {
                ...
                if (responseBody.containsKey("result")) {
                    ...
                    log.info("[RouterClient] Router 调用成功: resultLength={}", result.length());
                    return result;
                } else if (responseBody.containsKey("error")) {
                    String error = (String) responseBody.get("error");
                    log.warn("[RouterClient] Router 返回错误: {}", error);
                    return "❌ Router 错误: " + error;
                }
            }
            
            return "⚠️ Router 返回空结果";
            
        } catch (Exception e) {
            log.error("[RouterClient] Router 调用失败: {}", e.getMessage(), e);
            return "❌ 调用 Router 服务失败: " + e.getMessage();
        }
    }
