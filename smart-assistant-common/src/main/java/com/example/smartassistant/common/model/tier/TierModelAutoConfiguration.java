 ...
 * 仅注入名为 {@code deepSeekChatModel} 的 API 模型作为委托底层。</p>
 *
 * <p>仅在存在 {@code deepSeekChatModel} 时激活；无聊天模型的服务
...
public class TierModelAutoConfiguration {
     ...
     */
    @Bean
    public TierModelRegistry tierModelRegistry(@Qualifier("deepSeekChatModel") ChatModel apiModel,
                                               TieredModelRouterProperties props) {
        Map<ModelTier, TierModelRegistry.TierModelEntry> m = new EnumMap<>(ModelTier.class);
        m.put(ModelTier.LIGHT, entry(apiModel, props.getLight()));
        m.put(ModelTier.STANDARD, entry(apiModel, props.getStandard()));
        m.put(ModelTier.HEAVY, entry(apiModel, props.getHeavy()));
    ...
    private TierModelRegistry.TierModelEntry entry(ChatModel apiModel,
        ...
        return new TierModelRegistry.TierModelEntry(
                new DelegatingOptionsChatModel(apiModel, options), cfg.getModel());
    }
}
