 ...
 * 仅注入 {@link DeepSeekChatModel}（具体类型，唯一且明确）作为委托底层。</p>
 *
 * <p>仅在存在 {@link DeepSeekChatModel} 时激活；无模型 API 的环境（如纯 HTTP 转发的 user 服务）
...
public class TierModelAutoConfiguration {
     ...
     */
    @Bean
    public TierModelRegistry tierModelRegistry(DeepSeekChatModel deepSeek,
                                               TieredModelRouterProperties props) {
        Map<ModelTier, TierModelRegistry.TierModelEntry> m = new EnumMap<>(ModelTier.class);
        m.put(ModelTier.LIGHT, entry(deepSeek, props.getLight()));
        m.put(ModelTier.STANDARD, entry(deepSeek, props.getStandard()));
        m.put(ModelTier.HEAVY, entry(deepSeek, props.getHeavy()));
     ...
     *
     * <p>当 {@code tier.canary-model} 配置非空时，构建灰度模型（委托同一 DeepSeek API 实例、覆盖模型名与温度），
    ...
    public TieredModelRouter tieredModelRouter(TierModelRegistry registry,
        ...
        if (canaryName != null && !canaryName.isBlank()) {
            DeepSeekChatOptions canaryOptions = DeepSeekChatOptions.builder()
                    .model(canaryName)
                    .temperature(props.getCanaryTemperature())
                    .build();
            canaryModel = new DelegatingOptionsChatModel(deepSeek, canaryOptions);
    ...
    private TierModelRegistry.TierModelEntry entry(DeepSeekChatModel deepSeek,
        ...
        return new TierModelRegistry.TierModelEntry(
                new DelegatingOptionsChatModel(deepSeek, options), cfg.getModel());
    }
}
