...
// 数据库迁移：兼容旧版结构
var tryAddColumn = function (table, column, definition) {
    try {
        var info = db.prepare("PRAGMA table_info(".concat(table, ")")).all();
        if (!info.some(function (col) { return col.name === column; })) {
...
// 插入默认 FAQ 数据（如果为空）
var faqCount = db.prepare('SELECT COUNT(*) as cnt FROM faq').get().cnt;
if (faqCount === 0) {
    ...
    var insertMany = db.transaction(function () {
        for (var _i = 0, faqs_2 = faqs_1; _i < faqs_2.length; _i++) {
            var faq = faqs_2[_i];
            insertFaq_1.run.apply(insertFaq_1, faq);
        }
    });
    insertMany();
...
export function createSession(session) {
    ...
    stmt.run(session.id, session.title, session.model, session.sdk_session_id || null, session.intent || 'unknown', session.status || 'active', session.satisfaction || null, session.satisfaction_comment || null, session.user_name || '访客', session.agent_name || null, session.created_at, session.updated_at);
    return getSession(session.id);
}
export function updateSession(id, updates) {
    var fields = [];
    var values = [];
    var allowed = ['title', 'model', 'sdk_session_id', 'intent', 'status', 'satisfaction', 'satisfaction_comment', 'user_name', 'agent_name'];
...
export function createMessage(message) {
    ...
    stmt.run(message.id, message.session_id, message.role, message.content, message.model || null, message.intent || null, message.created_at, message.tool_calls || null);
...
export function getFaqsByCategory(category) {
    return db.prepare('SELECT * FROM faq WHERE category = ? ORDER BY hit_count DESC').all(category);
}
export function searchFaqs(query) {
    var likeQuery = "%".concat(query, "%");
    return db.prepare('SELECT * FROM faq WHERE question LIKE ? OR keywords LIKE ? OR answer LIKE ? ORDER BY hit_count DESC LIMIT 5').all(likeQuery, likeQuery, likeQuery);
}
export function incrementFaqHit(id) {
...
export function createFaq(faq) {
    ...
    return db.prepare('SELECT * FROM faq WHERE id = ?').get(faq.id);
}
export function updateFaq(id, updates) {
    var _a;
    var fields = [];
    var values = [];
    var allowed = ['question', 'answer', 'keywords', 'category'];
    for (var _i = 0, allowed_2 = allowed; _i < allowed_2.length; _i++) {
        var key = allowed_2[_i];
        if (key in updates) {
            fields.push("".concat(key, " = ?"));
            values.push(updates[key]);
        }
...
export function getSatisfactionStats() {
    var total = db.prepare('SELECT COUNT(*) as cnt FROM sessions').get().cnt;
    var rated = db.prepare('SELECT COUNT(*) as cnt FROM sessions WHERE satisfaction IS NOT NULL').get().cnt;
    var avgRow = db.prepare('SELECT AVG(satisfaction) as avg FROM sessions WHERE satisfaction IS NOT NULL').get();
    var scores = {};
    for (var i = 1; i <= 5; i++) {
        scores["score_".concat(i)] = db.prepare('SELECT COUNT(*) as cnt FROM sessions WHERE satisfaction = ?').get(i).cnt;
    }
    return __assign({ total: total, rated: rated, avg_score: avgRow.avg ? Math.round(avgRow.avg * 100) / 100 : null }, scores);
}
...
export function getTransferRate() {
    var total = db.prepare('SELECT COUNT(*) as cnt FROM sessions').get().cnt;
    if (total === 0)
        return 0;
    var transfers = db.prepare("SELECT COUNT(*) as cnt FROM sessions WHERE status = 'human_transfer'").get().cnt;
