...
public record ProductFeatureRequest(ProductFeatureConstraints constraints, String clarification) {
    ...
    private static final Pattern WEIGHT = Pattern.compile(
            "(?:不超过|不大于|最多|至多|<=|≤)" + NUMBER + WEIGHT_UNIT
    ...
    public static ProductFeatureRequest parse(String question) {
        String query = question == null ? "" : question.replaceAll("\\s+", "").toLowerCase(Locale.ROOT);
        // Saying a feature is not required is not a demand for its absence.
        query = query.replaceAll("(?:不需要|不要求|不关心|无需)(?:主动降噪|降噪|长续航|续航|轻便|便携)", "");
