...
class AgentCallerServiceTest {
    ...
    @Test
    @DisplayName("清理 OpenAI 兼容模型的 XML 思考区块")
    void testCleanThinkingContent_XmlThinking() throws Exception {
        Method method = AgentCallerService.class
                .getDeclaredMethod("cleanThinkingContent", String.class);
        method.setAccessible(true);

        AgentCallerService service = new AgentCallerService(null, null, null, null);
        String input = "<think>先分析热门商品和库存</think>当前热门商品如下";
        String result = (String) method.invoke(service, input);
        assertEquals("当前热门商品如下", result);
    }
