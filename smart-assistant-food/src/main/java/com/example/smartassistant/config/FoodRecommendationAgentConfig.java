...
public class FoodRecommendationAgentConfig {
    ...
    private String buildSystemPrompt() {
        return """
                ...
                2. Tool：根据需求选择合适的工具获取数据
                3. Review：检查上一步结果中是否包含人均价格、营业时间等时敏信息
                   若包含，在回答中标注"数据来自用户评价，实际以门店为准"
                ...
                ⚠️ 规则：仅当本次回答中调用了工具获取数据后，才在末尾生成 ⭐ 建议。
                 如果是无需工具调用的简单回复（如问候、感谢、闲聊），不生成任何建议。

                当需要生成时，换行后写 3-5 个后续建议，每行一个，格式：
                ⭐ 建议内容
                ⭐ 建议内容
                ⭐ 建议内容

                如果用户提问涉及具体地点，建议中必须包含一条引导出行规划的建议，例如引导用户规划行程。
